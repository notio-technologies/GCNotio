    ...
    context(context), site(site), service(service), baseURL(baseURL), clientsecret(clientsecret)
{
    ...
    // Used by Notio Konect to generate cryptographically secure random bytes.
    ...
    if (service) { // ultimately this will be the only way this works
        ...
        else if (service->id() == "Dropbox") site = this->site = DROPBOX;
        else if (service->id() == "Cycling Analytics") site = this->site = CYCLING_ANALYTICS;
        else if (service->id() == "Google Calendar") site = this->site = GOOGLE_CALENDAR;
        else if (service->id() == "Google Drive") site = this->site = GOOGLE_DRIVE;
        else if (service->id() == "University of Kent") site = this->site = KENTUNI;
        else if (service->id() == "Today's Plan") site = this->site = TODAYSPLAN;
        else if (service->id() == "Withings") site = this->site = WITHINGS;
        else if (service->id() == "PolarFlow") site = this->site = POLAR;
        else if (service->id() == "SportTracks.mobi") site = this->site = SPORTTRACKS;
        else if (service->id() == "Xert") site = this->site = XERT;
        else if (service->id() == "NotioKonect") site = this->site = NOTIOKONECT;
        else if (service->id() == "NotioCloud") site = this->site = NOTIOCLOUD;
        else site = this->site = NONE;
    }
    ...
    } else if (site == NOTIOKONECT) {

        // Generate a random alphanumeric key as a state.
        m_oAuthNkState = static_cast<NotioKonect*>(service)->getOAuthKey();

        // Generate a HMAC-SHA256 authentification code.
        static_cast<NotioKonect*>(service)->computeOAuthChallengCode(m_oAuthNkCodeVerifier, m_oAuthNkCodeChallenge);

        urlstr = QString("%1/api/v1/auth?").arg(NOTIOKONECT_HOST_URL);
        urlstr.append("client_id=").append(GC_NOTIOKONECT_CLIENT_ID).append("&");
        urlstr.append("state=").append(m_oAuthNkState).append("&");
        urlstr.append("code_challenge=").append(m_oAuthNkCodeChallenge);


    } else if (site == XERT) {
        urlChanged(QUrl("http://www.goldencheetah.org/?code=0"));
    }
    else if (site == NOTIOCLOUD) {
        // Generate a random alphanumeric key as a state.
        m_oAuthNkState = static_cast<NotioCloud*>(service)->getOAuthKey();

        // Generate a HMAC-SHA256 authentification code.
        static_cast<NotioCloud*>(service)->computeOAuthChallengCode(m_oAuthNkCodeVerifier, m_oAuthNkCodeChallenge);

        urlstr = QString("%1/oauth-ui/login.html?").arg(NotioCloud::kNotioApiBaseAddr);
        urlstr.append("client_id=").append(GC_NOTIO_CLIENT_ID).append("&");
        urlstr.append("state=").append(m_oAuthNkState).append("&");
        urlstr.append("code_challenge=").append(m_oAuthNkCodeChallenge);
    }
    ...
    if (site == DROPBOX || site == STRAVA || site == CYCLING_ANALYTICS || site == POLAR || site == SPORTTRACKS || site == GOOGLE_CALENDAR || site == GOOGLE_DRIVE || site == KENTUNI || site == TODAYSPLAN || site == WITHINGS
            || site == NOTIOKONECT || site == NOTIOCLOUD) {
...
OAuthDialog::urlChanged(const QUrl &url)
{
    ...
    if (site == DROPBOX || site == STRAVA || site == CYCLING_ANALYTICS || site == TODAYSPLAN || site == POLAR || site == SPORTTRACKS || site == XERT || site == WITHINGS
        ...
        if (url.toString().startsWith("http://www.goldencheetah.org/?state=&code=") ||
                ...
                url.toString().contains("success?auth_code=")) {

            QUrlQuery parse(url);
            QString code=parse.queryItemValue("code");

            // NotioCloud and NotioKonect are using the key "auth_code".
            if (code.isEmpty())
            {
                code = parse.queryItemValue("auth_code");
            }

            QString wState = parse.queryItemValue("state");
...
#endif
            QString urlstr = "";
            QJsonObject wJsonParams;        // NOTIOCLOUD use JSON format for token request.
            ...
            if (site == DROPBOX) {
                ...
                // Verify the state received by the host
                if (!wState.isEmpty() && (wState == m_oAuthNkState))
                {
                    urlstr = QString("%1/api/v1/token").arg(NOTIOKONECT_HOST_URL);
                    params.addQueryItem("client_id", GC_NOTIOKONECT_CLIENT_ID);
                    params.addQueryItem("client_secret", GC_NOTIOKONECT_CLIENT_SECRET);
                    params.addQueryItem("grant_type", "authorization_code");
                    params.addQueryItem("code_verifier", m_oAuthNkCodeVerifier);
                }
                else
                {
                    // The state code doesn't match the one sent initially.
                    QMessageBox wMsgbox(QMessageBox::Information, tr("Host not trusted"), tr("Host status response doesn't match client status."));
                    wMsgbox.exec();

                    // Reload the Notio Konect login page.
                ...
                if (!wState.isEmpty() && (wState == m_oAuthNkState))
                {
                    urlstr = QString("%1/oauth/v1/token").arg(NotioCloud::kNotioApiBaseAddr);
                    wJsonParams.insert("GrantType", "authorization_code");
                    wJsonParams.insert("AuthCode", code);
                    wJsonParams.insert("ClientSecret", GC_NOTIO_CLIENT_SECRET);
                    wJsonParams.insert("CodeVerifier", m_oAuthNkCodeVerifier);
                }
                else
                {
                    // The state code doesn't match the one sent initially.
            ...
            // Set data for all services except for NOTIOCLOUD.
            if (site != NOTIOCLOUD)
            {
#if QT_VERSION > 0x050000
                ...
                data.append(QJsonDocument(wJsonParams).toJson());
        ...
        // NOTIOKONECT authorization login failure from Notio Konect server.
        else if (url.toString().startsWith(NOTIOKONECT_HOST_URL) && url.toString().contains("failed?code="))
        {
            QString wErrorMessage;

            QUrlQuery parse(url);
            int wErrorCode = parse.queryItemValue("code").toInt();

            switch(wErrorCode)
            {
            case 1:
                wErrorMessage = tr("Email not found.");
                break;
            case 2:
                wErrorMessage = tr("Wrong password");
                break;
            case 3:
                wErrorMessage = tr("Access revoked, contact a system administrator");
                break;
            case 0:
            case 4:
            default:
                wErrorMessage = tr("Reason unknown, contact technical support.");
                break;
            }

            QString wMessageTitle = tr("Access denied");

            QMessageBox wMsgbox(QMessageBox::Information, wMessageTitle, wErrorMessage);
            wMsgbox.exec();
...
{
    printd("\nOAuthDialog::loadFinished only for Google Drive\n");
...
{
     printd("\nOAuthDialog::networkRequestFinished\n");
    ...
    if (reply->error() == QNetworkReply::NoError || reply->error() == QNetworkReply::SslHandshakeFailedError) {
        ...
        if (((site == GOOGLE_CALENDAR || site == GOOGLE_DRIVE || site == KENTUNI || site == NOTIOKONECT || site == NOTIOCLOUD) && refresh_token == "") || access_token == "") {

            // Something failed.
            // Google and NotioKonect use both refresh and access tokens.
        ...
        if (site == DROPBOX) {
            ...
            service->setSetting(GC_NOTIOKONECT_REFRESH_TOKEN, refresh_token.toUtf8().toBase64());
            service->setSetting(GC_NOTIOKONECT_ACCESS_TOKEN, access_token.toUtf8().toBase64());
            service->setSetting(GC_NOTIOKONECT_LAST_ACCESS_TOKEN_REFRESH, QDateTime::currentDateTime());
            ...
            // Saving logging date first. It is needed to encrypt and decrypt tokens.
            service->setSetting(GC_NOTIO_SIGN_IN, QDateTime::currentDateTime());

            // Save encoded access and refresh tokens.
            service->setSetting(GC_NOTIO_ACCESS_TOKEN, static_cast<NotioCloud *>(service)->encryptToken(access_token));
            service->setSetting(GC_NOTIO_REFRESH_TOKEN, static_cast<NotioCloud *>(service)->encryptToken(refresh_token));
            service->setSetting(GC_NOTIO_LAST_ACCESS_TOKEN_REFRESH, QDateTime::currentDateTime());
        }
