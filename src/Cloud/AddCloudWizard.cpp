...
AddSettings::initializePage()
{
    ...
    // Get the number of athletes. More than one means the account is owned by a coach.
    int wAthleteCount = wizard->cloudService->listAthletes().count();
    ...
    if (wizard->cloudService->capabilities() & CloudService::Query) {
        ...
        QString wToolTip = tr("Automatically download user's activities from the last 30 days.");
        bool wCanSync = true;

        // For Notio, coaches cannot do auto sync.
        if (wizard->cloudService->id().contains("Notio", Qt::CaseSensitive) && (wAthleteCount > 1))
        {
            // Uncheck and disable auto sync checkbox.
            value = "false";
            wCanSync = false;
            wToolTip = tr("A coach cannot automatically sync with the cloud.");
        }
        syncStartup->setChecked(value == "true");
        syncStartup->show();
        syncStartup->setEnabled(wCanSync);
        syncStartup->setToolTip(wToolTip);
    }
    if (wizard->cloudService->capabilities() & CloudService::Upload) {
        ...
        QString wToolTip = tr("Automatically import activities according to the import rules from athlete settings.");
        bool wCanImport = true;

        // For Notio, coaches cannot auto import activities.
        if (wizard->cloudService->id().contains("Notio", Qt::CaseSensitive) && (wAthleteCount > 1))
        {
            value = "false";
            wCanImport = false;
            wToolTip = tr("A coach cannot automatically import activities.");
        }
        syncImport->setChecked(value == "true");
        syncImport->show();
        syncImport->setEnabled(wCanImport);
        syncImport->setToolTip(wToolTip);
    }
}
