...
int CloudDBVersionClient::CloudDBVersion_Days_Delay = 5;
...
CloudDBVersionClient::informUserAboutLatestVersions() {
    ...
    QDate lastUpdateCheck = (appsettings->value(NULL, GC_LAST_NK_VERSION_CHECK_DATE, QDate(1990, 1, 1)).toDate());
    ...
    int lastVersion = appsettings->value(NULL, GC_LAST_NK_VERSION_CHECKED, 0).toInt();
    if (NK_VERSION_LATEST > lastVersion) {
        appsettings->setValue(GC_LAST_NK_VERSION_CHECKED, NK_VERSION_LATEST);
        lastVersion = NK_VERSION_LATEST;
    }
    ...
    appsettings->setValue(GC_LAST_NK_VERSION_CHECK_DATE, QDate::currentDate());
}
...
CloudDBVersionClient::showVersionPopup(QNetworkReply * l_reply) {
    ...
    // User ask manually to check for new versions.
    else if (m_manualCheckUpdates)
    {
        m_manualCheckUpdates = false;

        QMessageBox::information(nullptr, tr("GoldenCheetah - Check for new versions"), tr("No updates found."));
    }
}
...
void CloudDBUpdateAvailableDialog::doNotAskAgain() {
    ...
    if (versions.count()>0) {
        appsettings->setValue(GC_LAST_NK_VERSION_CHECKED, versions.at(0).Version);
    }
    accept();
}
