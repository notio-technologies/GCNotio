...
class CloudService : public QObject {
    ...
    public:
        ...
        explicit CloudService(Context *context);
        ...
        void setSyncDialog(CloudServiceSyncDialog *syncDiag) { syncDialog = syncDiag; }
        CloudServiceSyncDialog* getSyncDialog() const { return syncDialog; }
        ...
        virtual QList<CloudServiceEntry*> readdir(QString path, QStringList &errors) {
            Q_UNUSED(path);
            Q_UNUSED(errors);
            qDebug() << "readdir(path) not implemented.";
        ...
        virtual QList<CloudServiceEntry*> readdir(QString path, QStringList &errors, QDateTime from, QDateTime to) {
            Q_UNUSED(from);
            Q_UNUSED(to);
            qDebug() << "read dir (path,from,to) not implemented.";
    ...
    protected:
        ...
        void addCloudServiceEntry(CloudServiceEntry *p);
        QMap<QNetworkReply*,QString> replymap_;
        QList<CloudServiceEntry*> list_;

        Context *context;

        CloudServiceSyncDialog *syncDialog;

    public:
        virtual void uploadFileDialog() { return; }
        virtual void uploadFile(QString fileName) { Q_UNUSED(fileName); return; }
...
class CloudServiceSyncDialog : public QDialog
{
    ...
    private:
        ...
        QMap<RideItem*, bool> ridesInStore;
        ...
        QList<CloudServiceAthlete> athleteList;
...
class CloudServiceEntry
{
    public:
        ...
        // Notio
        QString type;                       // File type (Notio, FIT, merged)
        QString devices;                    // Device
        QString sport;                      // Sports
        ...
        void updateLocalInfo(QVector<RideItem*>& rides, Specification &specification, QMap<RideItem*, bool> &ridesInStore) {
            isLocal = false;
            for(int j=0; j<rides.count(); j++) {
                RideItem *ride = rides.at(j);
                if (!specification.pass(ride)) {
                    continue;
                }

                if(ride->fileName == name) {
                    isLocal= true;
                    ridesInStore.insert(ride, true);
                    break;
                }
            }

            if(!type.startsWith("merged;")) {
                QList<CloudServiceEntry*>::iterator it = children.begin();
                for( ;it != children.end(); ++it) {
                    (*it)->updateLocalInfo(rides, specification, ridesInStore);
                }
            }
        }
};
...
class CloudServiceFactory {
    ...
    void saveSettings(CloudService *service, Context *context) {
        ...
        while(want.hasNext()) {
            ...
            if (service->id().contains("Notio") == false)
            {
