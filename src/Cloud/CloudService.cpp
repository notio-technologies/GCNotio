...
CloudServiceEntry::CloudServiceEntry() {
    isDir = false;
    parent = NULL;
    initial = true;
    isLocal = false;
...
CloudService::CloudService(Context *context) :
    ...
    filetype(JSON), useMetric(false), useEndDate(false), context(context), syncDialog(NULL)
{
}
...
void
CloudService::addCloudServiceEntry(CloudServiceEntry *p)
{
    list_ << p;
}
    ...
    : QDialog(context->mainWindow, Qt::Dialog), context(context), store(store), downloading(false), aborted(false)
{
    ...
    store->setSyncDialog(this);
	
    bool notioSyncDialog = store->id().contains("Notio", Qt::CaseSensitive);
    ...
    // Set default tab. Notio services: Download Tab is the default.
    tabs->setCurrentIndex(notioSyncDialog ? 0 : 2);
    ...
    // Check if cloud service is capable of downloading data.
    bool wDownloadCapa = (store->capabilities() & CloudService::Download);
    tabs->setTabToolTip(0, wDownloadCapa ? QString(tr("Download from %1")).arg(store->uiName()) : tr("Download impossible."));

    // Check if cloud service is capable of uploading data.
    bool wUploadCapa = (store->capabilities() & CloudService::Upload);
    tabs->setTabEnabled(1, wUploadCapa);
    tabs->setTabToolTip(1, wUploadCapa ? QString(tr("Upload to %1")).arg(store->uiName()) : tr("Upload unavailable."));

    // Sync tab is not useful if neither upload or download capabilities is available.
    tabs->setTabEnabled(2, wDownloadCapa && wUploadCapa);
    tabs->setTabToolTip(2, (wDownloadCapa && wUploadCapa) ? QString(tr("Sync %1 and Golden Cheetah.")).arg(store->uiName()) : tr("Sync tab unavailable."));
    ...
    // Athlete Combo Box
    // Get athletes list.
    athleteList = store->listAthletes();
    QString wConfigAthleteIdKey = store->settings.value(CloudService::AthleteID, "");
    QString wCurrentAthleteID = store->getSetting(wConfigAthleteIdKey, "").toString();

    athleteCombo = new QComboBox(this);
    int wAthleteComboIndex = 0;

    // Cloud account has multiple athletes. This is a feature from Notio Konect.
    if (notioSyncDialog && !athleteList.empty())
    {
        // Populate the combo box.
        std::for_each(athleteList.begin(), athleteList.end(), [this, &wAthleteComboIndex, &wCurrentAthleteID](CloudServiceAthlete &wAthleteItr)
        {
            athleteCombo->addItem(wAthleteItr.name);

            // Determine the currently selected athlete.
            if (wAthleteItr.id == wCurrentAthleteID)
            {
                wAthleteComboIndex = athleteCombo->findText(wAthleteItr.name, Qt::MatchCaseSensitive);
            }
        });
    }
    // No athletes in the list, so we use the profile name.
    else
    {
        athleteCombo->addItem(context->athlete->cyclist);
    }

    athleteCombo->setCurrentIndex(wAthleteComboIndex);
	...
	{
        // Rename columns
        rideListDown->headerItem()->setText(1, tr("File"));
        rideListDown->headerItem()->setText(7, tr("Title"));
        rideListDown->header()->resizeSection(7, static_cast<int>(110 * dpiXFactor));

        // Add specific columns
        rideListDown->headerItem()->setText(8, tr("Duration"));
        rideListDown->header()->resizeSection(8, static_cast<int>(80 * dpiXFactor));
        rideListDown->headerItem()->setText(9, tr("Distance"));
        rideListDown->header()->resizeSection(9, static_cast<int>(80 * dpiXFactor));
        rideListDown->headerItem()->setText(10, tr("Sport"));
        rideListDown->header()->resizeSection(10, static_cast<int>(80 * dpiXFactor));
        rideListDown->headerItem()->setText(11, tr("Devices"));
        rideListDown->header()->resizeSection(11, static_cast<int>(150 * dpiXFactor));
        rideListDown->setColumnCount(12);

        // Hide workout cloud ID.
        rideListDown->hideColumn(6);
	}
    ...
    rideListDown->header()->resizeSection(4,20*dpiXFactor);
    rideListDown->header()->resizeSection(5,90*dpiXFactor);
    rideListDown->header()->resizeSection(6,50*dpiXFactor);
    rideListDown->header()->setStretchLastSection(false);
	...
	{
	    // Add specific columns.
        rideListUp->headerItem()->setText(8, tr("Title"));
        rideListUp->headerItem()->setText(9, tr("Sport"));
        rideListUp->header()->resizeSection(9, static_cast<int>(80 * dpiXFactor));
        rideListUp->headerItem()->setText(10, tr("Info"));
        rideListUp->setColumnCount(11);
	}
	else
	{
    ...
    rideListUp->header()->resizeSection(7,100*dpiXFactor);
    ...
    {
        // Add specific columns
        rideListSync->headerItem()->setText(9, tr("Workout Name"));
        rideListSync->headerItem()->setText(10, tr("Sport"));
        rideListSync->header()->resizeSection(10, static_cast<int>(80 * dpiXFactor));
        rideListSync->headerItem()->setText(11, tr("Devices"));
        rideListSync->headerItem()->setText(12, tr("Info"));
        rideListSync->setColumnCount(13);
        // Hide workout cloud ID.
        rideListDown->hideColumn(8);
    }
    ...
    rideListSync->header()->resizeSection(7,100*dpiXFactor);
    ...
    connect (from, SIGNAL(dateChanged(QDate)), this, SLOT(refreshClicked()));
    connect (to, SIGNAL(dateChanged(QDate)), this, SLOT(refreshClicked()));
    ...
    connect (athleteCombo, SIGNAL(currentIndexChanged(QString)), this, SLOT(userComboBoxChanged(QString)));
...
{
    // Disable refresh button to avoid refreshing to many times.
    ...
    foreach(RideItem *item, context->athlete->rideCache->rides()) {
        // Notio cloud uses activity cloud ID.
        if ((store->id() == "NotioCloud") && item->hasText("Cloud ID"))
            rideFiles << item->getText("Cloud ID", "");
        if (specification.pass(item))
            rideFiles << QFileInfo(item->fileName).baseName().mid(0,19);
    }

    bool notioSyncDialog = store->id().contains("Notio", Qt::CaseSensitive);

    // Legacy Notio Konect doesn't support duration nor distance display for Download Tab.
    if (store->id() == "NotioKonect")
    {
        rideListDown->hideColumn(8);
        rideListDown->hideColumn(9);
    }
    // Make sure duration and distance column are shown.
    else if (notioSyncDialog)
    {
        rideListDown->showColumn(8);
        rideListDown->showColumn(9);
    }
    ...
    for(int i=0; i<workouts.count(); i++) {
        ...
        add->setData(2, Qt::DisplayRole, ridedatetime.date());
        ...
        uploadFiles << targetnosuffix.mid(0,19);
        ...
        if (notioSyncDialog)
        {
            add->setText(7, workouts[i]->label);    // Ride title

            // Convert ride length in seconds into a Time Object.
            QTime wDuration(0, 0, 0);
            add->setText(8, wDuration.addSecs(workouts[i]->duration).toString("hh:mm:ss"));

            add->setText(9, QString("%1 km").arg(workouts[i]->distance, 0, 'f', 1));
            add->setText(10, workouts[i]->sport);
            add->setText(11, workouts[i]->devices);
        }

        if (rideFiles.contains(targetnosuffix.mid(0,19)) || rideFiles.contains(workouts[i]->id)) exists->setChecked(true);
        else {
            ...
            sync->setTextAlignment(1, Qt::AlignLeft);
            sync->setData(2, Qt::DisplayRole, ridedatetime.date());
            ...
            if (notioSyncDialog)
            {
                sync->setText(9, workouts[i]->label);
                sync->setText(10, workouts[i]->sport);
                sync->setText(11, workouts[i]->devices);
            }
        }
    }
    ...
    for(int i=0; i<context->athlete->rideCache->rides().count(); i++) {
        ...
        add->setData(2, Qt::DisplayRole, ride->dateTime.date());
        add->setTextAlignment(2, Qt::AlignLeft | Qt::AlignVCenter);
        ...
        if (notioSyncDialog)
        {
            add->setText(8, ride->getText("title", ""));
            add->setText(9, ride->getText("Sport", ""));
        }
        if (uploadFiles.contains(targetnosuffix.mid(0,19))) exists->setChecked(true);
        else {
            exists->setChecked(Qt::Unchecked);
            ...
            sync->setTextAlignment(1, Qt::AlignLeft);
            sync->setData(2, Qt::DisplayRole, ride->dateTime.date());
            sync->setTextAlignment(2, Qt::AlignLeft | Qt::AlignVCenter );
            ...
            if (notioSyncDialog)
            {
                sync->setText(8, ride->getText("title", ""));
                sync->setText(9, ride->getText("Sport", ""));
                sync->setText(12, reason);
            }
        }
    }
    ...
    rideListDown->sortItems(1, Qt::DescendingOrder);

    // refresh the progress label
    tabChanged(tabs->currentIndex());

    // Reactivate refresh button.
...
CloudServiceSyncDialog::syncNext()
{
    ...
    for (int i=listindex; i<rideListSync->invisibleRootItem()->childCount(); i++) {
        ...
        if (check->isChecked()) {
            ...
            if (curr->text(6) == tr("Download")) {
                ...
                bool result = store->readFile(data, curr->text(1), curr->text(8)); // filename
                if( result == false ) {
                    curr->setText(7, tr("Error on downloading"));
                }
                QApplication::processEvents();
                ...
                fullPathNme = context->athlete->home->activities().canonicalPath() + "/" + curr->text(1);
                QFile file(fullPathNme);
                RideFile *ride = RideFileFactory::instance().openRideFile(context, file, errors);
    ...
    // Pointing out to the user the status column when action unsuccessful.
    QString wSyncStatus = "";
    if (successful < downloadtotal)
    {
       wSyncStatus = tr("See Status column.");
    }
    progressLabel->setText(QString(tr("Processed %1 of %2 successfully. %3")).arg(successful).arg(downloadtotal).arg(wSyncStatus));
...
CloudServiceSyncDialog::downloadNext()
{
    for (int i=listindex; i<rideListDown->invisibleRootItem()->childCount(); i++) {
        ...
        if (check->isChecked()) {
            ...
            // Check if the file exist for Notio cloud service.
            RideItem *wRideToOverwrite = nullptr;
            if (store->id() == "NotioCloud")
            {
                for (auto &wRideItr : context->athlete->rideCache->rides())
                {
                    // For Notio cloud, we are refering to the cloud ID.
                    if (!wCloudId.isEmpty() && (wCloudId == wRideItr->getText("Cloud ID", "")))
                    {
                        // Since the date might differ, we need to remove the activity to act as overwrite.
                        wRideToOverwrite = wRideItr;
                        break;
                    }
                }
            }
            ...
            // Ride exist and have a different name. If overwrite checked, delete it to avoid duplicates.
            if (store->id() == "NotioCloud" && exists->isChecked() && overwrite->isChecked() &&
                    wRideToOverwrite && (wRideToOverwrite->fileName != curr->text(1)))
            {
                context->athlete->rideCache->removeRide(wRideToOverwrite);
            }
            return true;
        }
    }
    ...
    // Pointing out to the user the status column when action unsuccessful.
    QString wDownloadStatus = "";
    if (successful < downloadtotal)
    {
       wDownloadStatus = tr("See Status column.");
    }
    progressLabel->setText(QString(tr("Downloaded %1 of %2 successfully. %3")).arg(successful).arg(downloadtotal).arg(wDownloadStatus));
...
CloudServiceSyncDialog::completedRead(QByteArray *data, QString name, QString message)
{
    ...
    else if (message.contains(tr("Error")))
    {
        progressBar->setValue(++downloadcounter);

        QTreeWidgetItem *curr = which->invisibleRootItem()->child(listindex-1);
        curr->setText(col, message);
...
CloudServiceSyncDialog::uploadNext()
{
    for (int i=listindex; i<rideListUp->invisibleRootItem()->childCount(); i++) {
        ...
        if (check->isChecked()) {
            ...
            QStringList errors;
            QString fullPathNme  = context->athlete->home->activities().canonicalPath() + "/" + curr->text(1);
    ...
    // Pointing out to the user the status column when action unsuccessful.
    QString wUploadStatus = "";
    if (successful < downloadtotal)
    {
       wUploadStatus = tr("See Status column.");
    }
    progressLabel->setText(QString(tr("Uploaded %1 of %2 successfully. %3")).arg(successful).arg(downloadtotal).arg(wUploadStatus));
    return false;
}
...
CloudServiceSyncDialog::saveRide(RideFile *ride, QStringList &errors)
{
    ...
    if (store->id().contains("Notio", Qt::CaseSensitive))
	{
    ...
    else if (fileinfo.exists() && overwrite->isChecked() == false) {
        errors << tr("File exists");
        return false;
    }
    ...
    if (store->id().contains("Notio", Qt::CaseSensitive))
	{    
		context->athlete->addRide(fileinfo.fileName(), true, true, false, false);
    }
    else
...
CloudServiceAutoDownload::run()
{
    ...
    if (worklist.count()) {
        ...
        for(int i=0; i<worklist.count(); i++) {
            ...
            // No auto download is permitted with Notio Konect when a coach account is active.
            if (service->id().contains("Notio", Qt::CaseSensitive))
            {
                // Check if there are more than on athletes
                if (service->listAthletes().count() > 1)
                {
                    service->close();
                    delete service;
                    continue;
            ...
            if (found.count()) {
                ...
                foreach(RideItem *item, context->athlete->rideCache->rides()) {
                    if ((service->id() == "NotioCloud") && item->hasText("Cloud ID"))
                        rideFiles << item->getText("Cloud ID", "");
                    else if (specification.pass(item))
                        rideFiles << QFileInfo(item->fileName).baseName().mid(0,19);
                }
                ...
                foreach(CloudServiceEntry *entry, found) {
                    ...
                    {
                        if (entry->name.startsWith(name) || (entry->id == name))
                            got=true;
                    }
    ...
    for(int i=0; i<downloadlist.count(); i++) {
        ...
        QEventLoop loop;
        connect(this, SIGNAL(rideFileAdded()), &loop, SLOT(quit()));
...
///////////////////////////////////////////////////////////////////////////////////
/// \brief CloudServiceSyncDialog::userComboBoxChanged
///        This method is call upon selecting an athlete from the sync dialog combo
///        box. It select the athlete by its name from.
///
/// \param[in] iAthleteName Athlete's name.
///////////////////////////////////////////////////////////////////////////////////
void CloudServiceSyncDialog::userComboBoxChanged(QString iAthleteName)
{
    // Get the athlete based on the name from the combo box.
    for (auto wAthleteItr = athleteList.begin(); wAthleteItr != athleteList.end(); wAthleteItr++)
    {
        if (wAthleteItr->name == iAthleteName)
        {
            // Select athlete.
            store->selectAthlete(*wAthleteItr);
            break;
        }
    }
    refreshClicked();
