...
main(int argc, char *argv[])
{
    ...
    foreach (QString arg, sargs) {
...
#endif
            fprintf(stderr, "--development       to enable developpers only features.");
#ifdef GC_HAS_CLOUD_DB
        ...
        } else if ((arg == "--platform") || (arg.contains("windows:dpiawareness="))) {

            // This argument is only use for the system.
            continue;
#endif
        } else if (arg == "--debug") {
    ...
    QOperatingSystemVersion wCurrentOs = QOperatingSystemVersion::current();
    bool wIsWindows10 = (wCurrentOs.type() == QOperatingSystemVersion::Windows) ? (wCurrentOs >= QOperatingSystemVersion::Windows10) : false;

    // In Windows, we need to specified High Dpi support before creating the QApplication.
    if (wIsWindows10)
    ...
    if (desktop->screen()->devicePixelRatio() <= 1 && screenSize.width() > 2160 && (wIsWindows10 == false)) {
