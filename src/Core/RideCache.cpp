...
void
RideCache::removeRide(RideItem* todelete)
{
    ...
    for(index=0; index < rides_.count(); index++) {
       ...
       if (rides_[index]->fileName == todelete->fileName) {
    ...
    QString strOldFileName = todelete->fileName;

    QFile file((todelete->planned ? plannedDirectory : directory).canonicalPath() + "/" + strOldFileName);
