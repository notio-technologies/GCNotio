                                           ...
                                           QStringList &errors, QList<RideFile*> *rideList) const
{
    ...
    if (result) {
        ...
        // Change device type. We don't want BCVX to be displayed.
        if (result->deviceType().contains("BCVX"))
        {
            result->setDeviceType("Notio");
        }
        if (result->deviceType().contains("Notio"))
        {
            // Copy title in the notes section if empty.
            if (result->getTag("Notes", "").isEmpty())
            {
                result->setTag("Notes", result->getTag("title", ""));
            }
        }
...
// Data Series Iterator
DataSeriesIterator::DataSeriesIterator(XDataSeries *f, Specification spec, IterationSpec mode)
    : xDdata(f)/*, mode(mode)*/
{
    // index, start and stop are set to -1
    // if they are out of bounds or f is NULL
    if (f != NULL) {
        ...
        // ok, so lets work out the begin and end index
        double startsecs = spec.secsStart();
        //qDebug()<<"startseconds from spec "<<startsecs;
        if (startsecs < 0.0) { start = 0.0; }
        else { start = f->timeIndex(startsecs); }
        ...
        // ok, so lets work out the begin and end index
        double stopsecs = spec.secsEnd();
        //qDebug()<<"stopsecs from spec "<<stopsecs;
        if (stopsecs < 0.0) { stop = f->datapoints.count()-1;}
...
void DataSeriesIterator::toFront()
{
    index = start;
}

void DataSeriesIterator::toBack()
{
    index = stop;
}

XDataPoint * DataSeriesIterator::first()
{
    return start >= 0 ? xDdata->datapoints[start] : NULL; // efficient since dataPoints() returns a reference
}

XDataPoint * DataSeriesIterator::last()
{
    return stop >= 0 ? xDdata->datapoints[stop] : NULL; // efficient since dataPoints() returns a reference
}

bool DataSeriesIterator::hasNext() const
{
    return (index >= 0 && index <= stop);
}

bool DataSeriesIterator::hasPrevious() const
{
    return (index >= 0 && index >= start);
}

XDataPoint * DataSeriesIterator::next()
{
    if (index >= 0 && index <= stop) return xDdata->datapoints[index++];
    else return NULL;
}

XDataPoint * DataSeriesIterator::previous()
{
    if (index >= 0 && index >= start) return xDdata->datapoints[index--];
    else return NULL;
}
