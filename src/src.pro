...
HEADERS += $${LOCALHEADERS} \
    Cloud/NotioCloud.h \
    Cloud/NotioKonect.h \
    Charts/RideMap3DWindow.h \
    Charts/View3DFactory.h \
    Charts/RideMap3DPlot.h \
    FileIO/NotioComputeFunctions.h \
    FileIO/NotioData.h \
    Gui/ProgressMessageDialog.h \
    Gui/ChartsMngrDialog.h \
    Gui/ColorZonesBar.h
    
SOURCES += $${LOCALSOURCES} \
    Cloud/NotioCloud.cpp \
    Cloud/NotioKonect.cpp \
    Charts/RideMap3DWindow.cpp \
    Charts/View3DFactory.cpp \
    Charts/RideMap3DPlot.cpp \
    FileIO/NotioComputeFunctions.cpp \
    FileIO/NotioCorrectBCVX.cpp \
    FileIO/NotioCorrectDrops.cpp \
    Gui/ProgressMessageDialog.cpp \
    Gui/ChartsMngrDialog.cpp \
    Gui/ColorZonesBar.cpp \
    Metrics/NotioMetricAir.cpp
...
###=======================================================================
### NOTIO SPECIFIC DEPENDENCIES
###=======================================================================

!isEmpty(NOTIOALGO_INSTALL) {

    # We will work out the rest if you tell use where it is installed
    isEmpty(NOTIOALGO_INCLUDE) { NOTIOALGO_INCLUDE = $${NOTIOALGO_INSTALL}/include }
    isEmpty(NOTIOALGO_LIBS) {

        NOTIOALGO_LIBS = -L$${NOTIOALGO_INSTALL}/lib -lNotioAlgo
        win32 {
            CONFIG(debug, debug|release) {
                NOTIOALGO_LIBS = -L$${NOTIOALGO_INSTALL}/lib -lNotioAlgod
            }
        }
    }

    DEFINES     += GC_HAVE_NOTIOALGO
    INCLUDEPATH += $${NOTIOALGO_INCLUDE}
    LIBS        += $${NOTIOALGO_LIBS}

# Add header and source files using the algorithms.
    HEADERS += FileIO/NotioComputeProcessor.h Charts/NotioCDA.h Charts/NotioCDAWindow.h Charts/NotioVE.h \
               Charts/NotioVEWindow.h
    SOURCES += FileIO/NotioComputeProcessor.cpp Metrics/NotioMetricWind.cpp Metrics/NotioMetricCDA.cpp \
               Charts/NotioCDA.cpp Charts/NotioCDAWindow.cpp Charts/NotioVE.cpp Charts/NotioVEWindow.cpp
}
...
!isEmpty(ICAL_INSTALL) {
    ...
    # add caldav, caldavcloud and diary functions
...
###=====================
CONFIG(debug, debug|release) {
    # no optimistaions and no warning messages.
    #QMAKE_CXXFLAGS += -save-temps
    !win32-msvc*:QMAKE_CXXFLAGS += -g
    !win32-msvc*:QMAKE_CXXFLAGS += -O0
    QMAKE_CXXFLAGS -= -O1
    QMAKE_CXXFLAGS -= -O2
    QMAKE_CXXFLAGS -= -O3
    QMAKE_CXXFLAGS -= -Woverloaded-virtual
    QMAKE_CXXFLAGS -= -Wreorder
    QMAKE_CXXFLAGS -= -Wdeprecated-register
    !win32-msvc*:QMAKE_CXXFLAGS += -w
    QMAKE_CXXFLAGS -= -W
    win32-msvc*:QMAKE_CXXFLAGS += -W3
    QMAKE_CXXFLAGS -= -Wall
    #QMAKE_CXXFLAGS += -gline-tables-only
    message(QMAKE_CXXFLAGS(src debug): $$QMAKE_CXXFLAGS)
}

CONFIG(release, debug|release) {
    # no optimistaions and no warning messages.
    message(QMAKE_CXXFLAGS(src release): $$QMAKE_CXXFLAGS)
}
