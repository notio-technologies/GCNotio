...
public:
    ...
    // Source of metric
    enum metricsourcetype { Internal, XData, Notio };
    typedef enum metricsourcetype MetricSourceType;
    ...
    RideMetric() {
        ...
        value_ = 0.0;
        index_ = -1;
        sourceType_ = Internal;     // Needs to be set in metric's constructor inherited from RideMetric.
    }
    virtual ~RideMetric() {}
    ...
    virtual MetricSourceType sourceType() const { return sourceType_; }

    virtual QString XDataSeriesName() const { return xDataSeriesName_; }

    virtual QString XDataVariableName() const { return XDataVariableName_; }
    ...
    void setSourceType(MetricSourceType x) { sourceType_ = x; } // Important to set the metric's type in its constructor. Default is Internal.
    void setXDataSeriesName(const QString iName) { xDataSeriesName_ = iName; }
    void setXDataVariableName(const QString iName) { XDataVariableName_ = iName; }
    ...
    protected:
        ...
        QString xDataSeriesName_, XDataVariableName_;
        MetricType type_;
        MetricSourceType sourceType_;
};
...
private:
    ...
    QVector<RideMetric::MetricSourceType> metricSourceTypes;
    ...
    const RideMetric::MetricSourceType &metricSourceType(int i) const { return metricSourceTypes[i]; }
    ...
    void removeUserMetrics() {
        ...
        if (firstUser >0) {
            while (firstUser < metricNames.count()) {
                ...
                metricSourceTypes.remove(firstUser);
            }
        }
    }
    ...
    bool addMetric(const RideMetric &metric,
        ...
        metricSourceTypes.append(metric.sourceType());
        if (deps) {
