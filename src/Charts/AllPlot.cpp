...
///////////////////////////////////////////////////////////////////////////////
/// \brief AllPlot::parseAxisTitle
///        This method parse the Axis Title which might contain unit's name.
///
/// \param[in]  iAxisId Axis ID.
/// \param[out] oUnits  Output unit string.
/// \return The axis title.
///////////////////////////////////////////////////////////////////////////////
QString AllPlot::parseAxisTitle(const QwtAxisId &iAxisId, QString &oUnits)
{
    // Get the full axis title string.
    QString wAxisTitle = this->axisTitle(iAxisId).text();
    QString wReturning;

    // Get the unit name position if available.
    int wUnitPos = wAxisTitle.indexOf(" (");

    // Extract the title string.
    wReturning = wAxisTitle.left(wUnitPos);

    // Extract units from axis title.
    if (wUnitPos >= 0)
    {
        oUnits = wAxisTitle.mid(wUnitPos + 2);
        oUnits.remove(')');
    }
    return wReturning;
}
...
AllPlot::setDataFromPlot(AllPlot *plot)
{
    ...
    QString title;
    QString wUnits = RideFile::unitName(scope, context);
    ...
    if ((ourCurve && thereCurve) || (ourICurve && thereICurve) || (ourASCurve && thereASCurve)) {
        ...
        if (scope == RideFile::thb || scope == RideFile::smo2
            ...
            // Modifies the accuracy of the scale values displayed on the Y axis when
            // the curve varies with a small range.
            double wMaxValue = 100.0;
            double wMinValue = 0.0;

            // Get the maximum and minimum values for the curve.
            if (thereCurve) {
                wMinValue = thereCurve->minYValue();
                wMaxValue = 1.1 * thereCurve->maxYValue();
            } else if (thereICurve) {
                wMinValue = thereICurve->boundingRect().top();
                wMaxValue = 1.1 * thereICurve->boundingRect().bottom();
            } else if (thereASCurve) {
                wMinValue = thereASCurve->minYValue();
                wMaxValue = 1.1 * thereASCurve->maxYValue();
            }

            // Evaluate the range of the values.
            double wValueDelta = abs(wMaxValue - wMinValue);
        ...
        // title, units and colour
        if (wUnits.isEmpty() == false)
            title.append(QString(" (%1)").arg(wUnits));
...
AllPlot::pointHover(QwtPlotCurve *curve, int index)
{
    ...
    if (index >= 0 && curve != standard->intervalHighlighterCurve && 
        ...
        if (curve->title() == tr("Speed") && rideItem) {
            ...
            {
                // Add to tooltip headwind value relative to the rider speed.
                if ((standard != nullptr) && showWind && (static_cast<size_t>(index) < standard->windCurve->data()->size()))
                {
                    // Get Headwind curve minimum and maximum values.
                    double wHeadWindMax = standard->windCurve->sample(index).interval.maxValue();
                    double wHeadWindMin = standard->windCurve->sample(index).interval.minValue();

                    // Head wind.
                    if (wHeadWindMax > yvalue)
                    {
                        paceStr = QString("\n%1\n%2").arg(tr("Head wind"), QString::number(wHeadWindMax - yvalue, 'f', 2));
                    }
                    // Tail wind.
                    else if (wHeadWindMin < yvalue)
                    {
                        paceStr = QString("\n%1\n%2").arg(tr("Tail wind"), QString::number(yvalue - wHeadWindMin, 'f', 2));
                    }

                    // Wind units.
                    if (paceStr.isEmpty() == false)
                        paceStr.append(" " + (context->athlete->useMetricUnits ? tr("kph") : tr("mph")));
                }
            }
        ...
        QString text = QString("%1\n%2 %3%4\n%5 %6")
                        .arg(parseAxisTitle(curve->yAxis(), wUnits))
