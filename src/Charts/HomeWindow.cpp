    ...
    clicked(NULL), dropPending(false), chartCursor(-2), loaded(false)
{
    ...
    tabLayout->addWidget(new CustomProgressLine(this, context));    // Data processor progress bar.
...
///////////////////////////////////////////////////////////////////////////////////
/// \brief HomeWindow::updateChartsOrderLayout
///        This method is used to update the charts order.
///////////////////////////////////////////////////////////////////////////////////
void HomeWindow::updateChartsOrderLayout()
{
    // Order changed.
    chartOrderChanged = true;

    // Call styleChanged with the current style just to refresh.
    styleChanged(currentStyle);
}

///////////////////////////////////////////////////////////////////////////////////
/// \brief HomeWindow::getName
///        This method return the view display tab name.
///
/// \return The name displayed.
///////////////////////////////////////////////////////////////////////////////////
QString HomeWindow::getName()
{
    if (name == "diary")
        return tr("Diary");
    else if (name == "home")
...
void
HomeWindow::tabMoved(int from, int to)
{
    // re-order the tabs
...
{
    // ignore if out of bounds or we're already using that style except if the charts order changed.
    if ((id > 2 || id < 0 || id == currentStyle) && !chartOrderChanged) return;
    active = true;
    chartOrderChanged = false;  // Reset flag.
...
GcWindowDialog::GcWindowDialog(GcWinID type, Context *context, GcChartWindow **here, bool sidebar, LTMSettings *use) :
    context(context), type(type), here(here), sidebar(sidebar)
{
...
void
HomeWindow::saveState(bool iManualSave)
{
    ...
    QString filename = context->athlete->home->config().canonicalPath() + "/" + name + "-notio-layout.xml";

    // Check if the user want to backup the view's layout on the disk.
    if (iManualSave)
    {
        filename = QFileDialog::getSaveFileName(this, tr("Save Layout"), QDir::homePath()+"/"+ name + "-layout.xml", ("*.xml;"));

        // No file name or cancel button has been pressed.
        if (filename.isEmpty())
            return;
    }
...
{
    QString filename = context->athlete->home->config().canonicalPath() + "/" + name + "-notio-layout.xml";
    QString wOldNotioLayout = context->athlete->home->config().canonicalPath() + "/" + name + "-notiokonect-layout.xml";

    // Rename old Notio Konect layout configuration file.
    if (QFileInfo(wOldNotioLayout).exists())
    {
        QFile::rename(wOldNotioLayout, filename);
    }
...
///////////////////////////////////////////////////////////////////////////////
/// \brief HomeWindow::importLayout
///        This method imports a layout for the current view selected by the
///        user.
///////////////////////////////////////////////////////////////////////////////
void HomeWindow::importLayout()
{
    // Prompt user to select a layout to import.
    QString filename = QFileDialog::getOpenFileName(this, tr("Import Layout"), QDir::homePath(), ("*.xml;"));
    QFileInfo finfo(filename);
    ...
    // If we don't have content then something went
    // wrong, so only import if its not blank.
    if (content != "")
    {
        // Setup the handler.
        QXmlInputSource source;
        ...
        // Verify that the select layout is for the current view.
        if (handler.viewName == name)
        {
            // Whilst this happens don't show user.
            setUpdatesEnabled(false);
            ...
            // Translate the metrics, but only for LTM charts and only if the language
            // is not English (i.e. translation is required).
            if (!english)
            {
                // Translate the titles.
                translateChartTitles(handler.charts);
    ...
    // Set to whatever we have selected.
    for(int i = 0; i < charts.count(); i++) {
        RideItem *notconst = const_cast<RideItem *>(context->currentRideItem());
        charts[i]->setProperty("ride", QVariant::fromValue<RideItem*>(notconst));
        charts[i]->setProperty("dateRange", property("dateRange"));
        if (currentStyle != 0) charts[i]->show();
    ...
    if (currentStyle == 0 && charts.count())
        tabSelected(0);
}
...
{
    if (qName == "chart" && (chart != NULL)) { // add to the list
        charts.append(chart);
    }
    return true;
}
...
bool ViewParser::startElement( const QString&, const QString&, const QString &name, const QXmlAttributes &attrs )
{
    if (name == "layout") {
        for(int i=0; i<attrs.count(); i++) {
            ...
            else if (attrs.qName(i) == "name") {
                viewName = Utils::unprotect(attrs.value(i));
            }
        }
    }
    else if (name == "chart") {
    ...
    else if (name == "property" && (chart != NULL)) {
...
void HomeWindow::translateChartTitles(QList<GcChartWindow*> charts)
{
    ...
    titleMap.insert("3D Map", tr("3D Map"));
    titleMap.insert("Notio CdA Analysis", tr("Notio CdA Analysis"));
    titleMap.insert("Environment", tr("Environment"));
    titleMap.insert("Physiological", tr("Physiological"));
    titleMap.insert("Efficiency", tr("Efficiency"));
    titleMap.insert("Mechanical", tr("Mechanical"));
    titleMap.insert("Notio Bike Dynamics", tr("Notio Bike Dynamics"));
