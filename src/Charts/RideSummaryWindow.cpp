...
#define NOTIO_MOVE_INTERVALS_SECTION
     ...
     GcChartWindow(context), context(context), ridesummary(ridesummary), useCustom(false), useToToday(false), filtered(false), bestsCache(NULL), force(false)
{
    ...
    // For Notio rides, we add a button on the summary page as a shortcut to compute.
    // Object that will link the html button to the code.
    m_computeButton = new htmlButtonBridge;
    connect(m_computeButton, SIGNAL(clicked()), context->mainWindow, SLOT(NotioMenuCompute()), Qt::QueuedConnection);

#ifdef NOWEBKIT
    m_htmlSummaryChannel = new QWebChannel(rideSummary->page());
    m_htmlSummaryChannel->registerObject(QStringLiteral("computeButton"), m_computeButton);
    rideSummary->page()->setWebChannel(m_htmlSummaryChannel);
#endif
#endif
    ...
    if (ridesummary) {
        ...
        // When changing or downloading ride, the signal is emitted too many time in so little time. The summary
        // page content then refresh too fast and doesn't give enough time for the previous content to close
        // causing an issue with the javascript used for the Notio compute button. The issue still occurs from
        // time to time. Updating to a newer QT version might resolve it.
        connect(this, SIGNAL(rideItemChanged(RideItem*)), this, SLOT(rideItemChanged()), Qt::QueuedConnection);
...
#ifdef GC_HAVE_NOTIOALGO
///////////////////////////////////////////////////////////////////////////////
/// \brief RideSummaryWindow::computeNotio
///        This method is called when the summary page compute button has been
///        clicked to start Notio ride data computation.
///////////////////////////////////////////////////////////////////////////////
void RideSummaryWindow::computeNotio()
{
    // Close summary page.
    rideSummary->triggerPageAction(QWebEnginePage::RequestClose);

    // Compute ride.
    context->mainWindow->NotioMenuCompute();
}
#endif
...
RideSummaryWindow::htmlSummary()
{
...
#ifdef NOWEBKIT
#ifdef GC_HAVE_NOTIOALGO
    // Notio ride file but hasn't been computed yet.
    QString wMsgDummy;
    ...
    if (ridesummary && !NotioFuncCompute::speedPowerUnavailable(ride, wMsgDummy) && (!ride->xdata("CDAData") || wNeedToRecompute))
    {
        // Create a button to compute the ride and link it through javascript.
        summary += QString("<script type=\"text/javascript\" src=\"qrc:///qtwebchannel/qwebchannel.js\"></script>\n");
        summary += QString("<script type=\"text/javascript\">                                   \n"
                           "var computeButton;                                                  \n"
                           "var clickedMsg = \"Compute button clicked.\";                       \n"
                           "window.onload = function () {                                       \n"
                           "    <!-- Map compute button object to the script. -->               \n"
                           "    new QWebChannel(qt.webChannelTransport, function (channel) {    \n"
                           "        computeButton = channel.objects.computeButton;              \n"
                           "    });                                                             \n"
                           "    console.log(\"Get button bridge for ride %1.\");                \n"
                           "};                                                                  \n"
                           "window.onunload = function () {                                     \n"
                           "    console.log(\"Unloading ride %1 summary.\");                    \n"
                           "};                                                                  \n"
                           "</script>                                                           \n"
                           ).arg(ride->startTime().toString("yyyy_MM_dd_hh_mm_ss"));

        // Add message and button for the user to compute data.
        if (wNeedToRecompute)
        {
            summary += tr("<h4>You should recompute to avoid any data inconsistencies.</h4>");
            summary += QString("<button class=\"button\" onclick=\"{ console.log(clickedMsg); computeButton.onClick(); }\">%1</button>").arg(tr("Recompute"));
        }
        else
        {
            summary += tr("<h4>You need to compute your Notio device data to see your CdA.</h4>");
            summary += QString("<button class=\"button\" onclick=\"{ console.log(clickedMsg); computeButton.onClick(); }\">%1</button>").arg(tr("Compute"));
        }
        summary += QString("<style> .button {                       "
                           "            padding: 10px 20px;         "
                           "            border: none;               "
                           "            text-align: center;         "
                           "            border-radius: 3px;         "
    ...
    // Move interval section for Notio.
#ifdef NOTIO_MOVE_INTERVALS_SECTION
    ...
    // Move interval section for Notio.
#ifndef NOTIO_MOVE_INTERVALS_SECTION
    // Only get interval summary for a ride summary
    if (ridesummary) {
...
///////////////////////////////////////////////////////////////////////////////
/// \brief htmlButtonBridge::onClick
///        This method is invoked by a html button on the summary page. It
///        links the button with GC interface.
//////////////////////////////////////////////////////////////////////////////
void htmlButtonBridge::onClick()
{
    emit clicked();
}
