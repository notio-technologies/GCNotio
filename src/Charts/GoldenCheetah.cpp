...
GcChartWindow::exportChartToCloudDB()
{
    ...
    if ( chart.ChartType.toInt() == GcWindowTypes::RideSummary ||
         ...
         chart.ChartType.toInt() == GcWindowTypes::NotchPlayer )
    {
    ...
    // writes pixmap into bytes in PNG format (a bit larger than JPG - but much better in Quality when importing)
    QString wPictureFormat = "PNG";

    // The 3D map picture takes too much bytes in PNG.
    if (chart.ChartType.toInt() == GcWindowTypes::RideMap3DWindow)
        wPictureFormat = "JPG";
    ...
    picture.save(&buffer, wPictureFormat.toStdString().c_str());
    buffer.close();
