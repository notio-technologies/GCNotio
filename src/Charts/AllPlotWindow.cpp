...
{
    setProperty("ClassName", "AllPlotWindow");
    createNewInterval = false;
    toCheckIntegrity = false;
    ...
    overlayIntervals = new IntervalSummaryWindow(this, context);
    ...
    if (context->isCompareIntervals) {
        ...
        foreach(SeriesWanted x, wanted) {
            ...
            if (x.one == RideFile::alt && x.two == RideFile::slope) {
                ...
                // Get unit name.
                QString wUnits = RideFile::unitName(x.one, context);

                // Set title.
                QString wTitle = tr("Alt/Slope");

                // Add units to title if available.
                if (wUnits.isEmpty() == false)
                    wTitle.append(QString(" (%1)").arg(wUnits));

                // alt/slope special case
                plot->setAxisTitle(QwtPlot::yLeft, wTitle);
                ...
                if (x.one > RideFile::none) {
                    ...
                    // Get unit name.
                    QString wUnits = userDataSeries[index]->units;
                    QString wTitle = userDataSeries[index]->name;
                    ...
                    case RideFile::lat:
                    case RideFile::none:
                        // Don't put units for these curves.
                        break;
                    default:
                        wUnits = RideFile::unitName(x.one, context);
                        break;
                    }

                    // Get series name.
                    QString wTitle = RideFile::seriesName(x.one);
...
AllPlotWindow::intervalsChanged()
{
    ...
    QMap<RideItem*, QMap<unsigned int, bool> >::ConstIterator it = mapRideIntervalNewName.find(current);
    if(it != mapRideIntervalNewName.end()) {
        QList<QString> intervalNames;
        for(auto e : current->intervals()) {
            intervalNames.append(e->name);
        }

        QMap<unsigned int, bool> valueMap = it.value();
        for(QMap<unsigned int, bool>::ConstIterator itValMap = valueMap.begin(); itValMap != valueMap.end(); ++itValMap) {
            unsigned int tag = itValMap.key();
            QString nameEx = QString(tr("Selection #%1 ")).arg(tag);

            if(!intervalNames.contains(nameEx)) {
                valueMap.insert(tag, false);
            }
        }

        mapRideIntervalNewName.insert(current, valueMap);
    }
...
AllPlotWindow::plotPickerMoved(const QPoint &pos)
{
    ...
    if (showStack->isChecked()) {
        ...
        foreach (AllPlot *_plot, (showBySeries->isChecked() ? seriesPlots : allPlots)) {
            ...
            if (_plot->y()<=plot->y() && posY<_plot->y()){
                ...
                if (_plot->transform(QwtPlot::xBottom, _plot->standard->allMarker2->xValue())<plot->width()){
                    setEndSelection(_plot, _plot->transform(QwtPlot::xBottom, plot->width()), false);
                }
            }
                ...
                setEndSelection(_plot, _plot->invTransform(QwtPlot::xBottom, posX), true);
        ...
        // working on AllPlot
        double xValue = plot->invTransform(QwtPlot::xBottom, pos.x());
        setEndSelection(plot, xValue, true);
    }
}
...
void
AllPlotWindow::setEndSelection(AllPlot* plot, double xValue, bool newInterval)
{
    // we don't do selection in compare mode
    if (isCompare()) return;
    ...
    if (!allMarker2->isVisible() || allMarker2->xValue() != xValue) {
        ...
        if (newInterval) {
            if(createNewInterval) {
                ...
                if(mapRideIntervalNewName.contains(ride)) {
                    nextTag = getNextSelectionId(ride);
                }
                else {
                    nextTag = 1;
                    QMap<unsigned int, bool> mapIntervalNames;
                    mapIntervalNames.insert(nextTag, true);
                    mapRideIntervalNewName.insert(ride, mapIntervalNames);
                }

                QString nameEx = QString(tr("Selection #%1 ")).arg(nextTag);

                //create a new one
                currentInterval = ride->newInterval(nameEx, duration1, duration2, distance1, distance2, Qt::black, false);
                currentInterval->selected = true;
                ...
                if(currentInterval) {
                    currentInterval->setValues(currentInterval->name, duration1, duration2, distance1, distance2, currentInterval->color, false);
                }
                else {
                    QMessageBox::warning(this, tr("current intervall null"), tr("current intervall null!!!!!"));
                }
            }
...
AllPlotWindow::clearSelection()
{
    ...
    if(!mapRideIntervalNewName.contains(current)) {
        // First ride selection
        QMap<unsigned int, bool> iTags;
        for(auto e : current->intervals()) {
            if(e->name.startsWith("Selection #")){
                // get tag
                QString name = e->name;
                name = name.trimmed();
                QStringList listStr = name.split("#");
                iTags.insert(listStr[1].toInt(), true);
            }
        }

        mapRideIntervalNewName.insert(current, iTags);
    }
...
unsigned int
AllPlotWindow::getNextSelectionId(RideItem *currentRide)
{
    unsigned int toUse = 0;

    if(mapRideIntervalNewName.contains(currentRide)) {
        QMap<unsigned int, bool> valueMap = *(mapRideIntervalNewName.find(currentRide));
        QMap<unsigned int, bool>::Iterator it = valueMap.begin();
        ...
        // if not found or all deleted -> set to 1
        toUse++;

        mapRideIntervalNewName.find(currentRide)->insert(toUse, true);
    }
    else {
        // create new value map
        QMap<unsigned int, bool> mapIntervalNames;
        toUse = 1;
        mapIntervalNames.insert(toUse, true);
        mapRideIntervalNewName.insert(currentRide, mapIntervalNames);
    }
...
unsigned int
AllPlotWindow::getLastSelectionId(RideItem *currentRide)
{
    unsigned int lastUsed = 0;

    if(mapRideIntervalNewName.contains(currentRide)) {
        QMap<unsigned int, bool> valueMap = *(mapRideIntervalNewName.find(current));//.last();
        for(QMap<unsigned int, bool>::Iterator it = valueMap.begin(); it != valueMap.end(); ++it) {
            if(it.value() == true) {
                lastUsed = it.key();
            }
        }
...
void
AllPlotWindow::checkIntervalsIntegrity() {

    // loop through the intervals for this tree
    for(auto e : current->intervals()) {
        if ( e->type == RideFileInterval::USER && ! e->rideInterval) {
            //QMessageBox::warning(this, tr("Error Interval"), tr("No ride Interval!!!!!"));
            qDebug()<<"Warning: No ride interval saved for intrval "<<e->name<<". Should save Ride after create/delete new intervals.";
        }
    }
}
