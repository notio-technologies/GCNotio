...
HelpWhatsThis::getText(GCHelp chapter) {
    ...
    // Notio device data.
    case MenuBar_Edit_FixBCVxUsingGarmin:
        return text.arg("Menu%20Bar_Edit").arg(tr("Fix BCVx using Gamin"));
    case MenuBar_Edit_FixNotioPowerDrops:
            return text.arg("Menu%20Bar_Edit").arg(tr("Fix BCVx Power Drops"));
    case MenuBar_Edit_FixNotioSpeedDrops:
            return text.arg("Menu%20Bar_Edit").arg(tr("Fix BCVx Speed Drops"));
    case MenuBar_Edit_NotioComputeData:
        return text.arg("Menu%20Bar_Edit").arg(tr("Compute Notio Data"));
    ...
    case ChartRides_Map3D:
        return text.arg("ChartTypes_Activities#cesium-3d-map").arg(tr("3D Map of activity"));
    case ChartRides_Map3D_Config_Basic:
        return text.arg("ChartTypes_Activities#map3D-basic").arg(tr("Selection of the data series to be shown and general settings on the 3D map structure."));
    case ChartRides_Map3D_Config_UserData:
        return text.arg("ChartTypes_Activities#map3D-userdata").arg(tr("Creation of user data series to be added to series selection list."));
    case ChartRides_2D:
    ...
    case FindIntervals_Err_AeroTest_SampleRate:
        return text.arg("Side-Bar_Activities-view#intervals")
                .arg(tr("The sample rate of the ride is not conform with the recording standard. Make sure Notio firmware is up-to-date with the official version to use this interval method."));
    case FindIntervals_Err_AeroTest_Min:
        return text.arg("Side-Bar_Activities-view#intervals")
                .arg(tr("Minimum ride distance for out or back is not respected. Make sure you ride for at least 1.5 km before turning around and returning to your starting point."));
    case FindIntervals_Err_AeroTest_Distance:
        return text.arg("Side-Bar_Activities-view#intervals")
                .arg(tr("We were not able to detect the turnaround point. Make sure you ride for at least 1.5 km before turning around and returning to your starting point."));
    case FindIntervals_Err_AeroTest_Altitude:
        return text.arg("Side-Bar_Activities-view#intervals")
                .arg(tr("Road grade inconsistencies were detected when comparing the out and back intervals. Try and find a mostly flat terrain that is at least 1.5 km long and restart the test."));
    case FindIntervals_Err_AeroTest_Wind:
        return text.arg("Side-Bar_Activities-view#intervals")
                .arg(tr("Wind turbulence detected between out and back intervals. Turbulence generated by passing cars, wind gusts or lateral winds can affect the reliability of the results. Restart the test later when conditions are back to normal."));
    case FindIntervals_Err_AeroTest_Sensors:
        return text.arg("Side-Bar_Activities-view#intervals")
                .arg(tr("Power meter and speed sensor disconnections where noticed affecting the reliability of the results. Make sure your sensors are well charged and connected."));
    case FindIntervals_Err_AeroTest_Out_Back:
        return text.arg("Side-Bar_Activities-view#intervals")
                .arg(tr("Cannot find out and back intervals."));
    case FindIntervals_Err_AeroTest_Data:
        return text.arg("Side-Bar_Activities-view#intervals")
                .arg(tr("The necessary data used to find aero test laps is missing. Re-importing ride, and computing it, might fix the issue."));
    case FindIntervals_Err_Velodrome_Distance:
        return text.arg("Side-Bar_Activities-view#intervals")
                .arg(tr("The length of the laps found diverged too much."));
    case FindIntervals_Err_Velodrome_NoLaps:
        return text.arg("Side-Bar_Activities-view#intervals")
                .arg(tr("Impossible to find lap from the data."));
    case FindIntervals_Err_Velodrome_Speed:
        return text.arg("Side-Bar_Activities-view#intervals")
                .arg(tr("Speed is not stable enough."));
    case FindIntervals_Err_Velodrome_Acceleration:
        return text.arg("Side-Bar_Activities-view#intervals")
                .arg(tr("Acceleration is not stable enough."));
    case FindIntervals_Err_Velodrome_Data:
        return text.arg("Side-Bar_Activities-view#intervals")
                .arg(tr("The necessary data used to find velodrome laps is missing. Re-importing ride, and computing it, might fix the issue."));
#endif
    ...
    case Preferences_Athlete_ChartManager:
        return text.arg("Preferences_Athlete_ChartManager").arg(tr("Charts Order"));
