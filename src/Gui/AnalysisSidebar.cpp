...
{
    setProperty("ClassName", "AnalysisSidebar");
    ...
    intervalSummaryWindow = new IntervalSummaryWindow(this, context);
...
AnalysisSidebar::intervalPopup()
{
    ...
    if (rideItem && rideItem->intervalsSelected(RideFileInterval::USER).count() == 1) {
        ...
        QAction *actCopyInt = new QAction(tr("Duplicate interval"), intervalTree);
        ...
        connect(actCopyInt, SIGNAL(triggered(void)), this, SLOT(duplicateIntervalSelected(void)));
        menu.addAction(actEditInt);
        menu.addAction(actCopyInt);
    ...
    if (rideItem && rideItem->intervalsSelected(RideFileInterval::USER).count() > 1) {
        ...
        connect(actDeleteInt, SIGNAL(triggered(void)), this, SLOT(deleteIntervalSelected(void)));
        QAction *actCopyInt = new QAction(tr("Duplicate selected intervals"), intervalTree);
        connect(actCopyInt, SIGNAL(triggered(void)), this, SLOT(duplicateIntervalSelected(void)));
...
///////////////////////////////////////////////////////////////////////////////////
/// \brief AnalysisSidebar::duplicateIntervalSelected
///        This method creates copies of selected user defined intervals.
///////////////////////////////////////////////////////////////////////////////////
void AnalysisSidebar::duplicateIntervalSelected()
{
    QTreeWidgetItem *wUserIntervals = trees.value(RideFileInterval::USER, NULL);

    // Check if we are duplicating intervals.
    if (context->currentRideItem() && wUserIntervals && wUserIntervals->childCount() &&
        context->currentRideItem()->intervalsSelected(RideFileInterval::USER).count()) {

        QString wPrefixString = tr("Copy of");

        // Go through all selected intervals.
        for(int j = 0; j < wUserIntervals->childCount(); j++) {

            // Get the pointer to the IntervalItem for this item
            QVariant wVariant = wUserIntervals->child(j)->data(0, Qt::UserRole);

            // Make the IntervalItem selected flag reflect the current selection state
            IntervalItem wItem = *(static_cast<IntervalItem*>(wVariant.value<void*>()));

            // Is it selected and linked ?
            if (wItem.selected && wItem.rideInterval) {

                // Set item's name with the prefix "Copy of"
                wItem.name = QString("%1 %2").arg(wPrefixString).arg(wItem.name);

                // Create a new interval item based on the original.
                const_cast<RideItem*>(context->currentRideItem())->newInterval(wItem.name, wItem.start, wItem.stop, wItem.startKM, wItem.stopKM, Qt::black, false);
            }
        }

        // Mark as dirty and tell the charts it changed.
        const_cast<RideItem*>(context->currentRideItem())->setDirty(true);
        context->notifyIntervalsUpdate(const_cast<RideItem*>(context->currentRideItem()));
    }
}
