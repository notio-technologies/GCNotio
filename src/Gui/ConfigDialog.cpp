    ...
    home(home), context(context)
{
    ...
    chartsPage = new ChartMngrPage(context);
    HelpWhatsThis *chartManagerHelp = new HelpWhatsThis(chartsPage);
    chartsPage->setWhatsThis(chartManagerHelp->getWhatsThisText(HelpWhatsThis::Preferences_Athlete_ChartManager));
    ...
    tabs->addTab(chartsPage, tr("Charts Order"));
...
qint32 MetricConfig::save()
{
    ...
    if(context != NULL && context->rideItem() != NULL) {
        foreach(IntervalItem *ii, context->rideItem()->intervals()) {
            if(ii != NULL) {
                ii->refresh();
            }
        }
