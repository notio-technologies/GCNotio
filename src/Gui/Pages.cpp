...
GeneralPage::GeneralPage(Context *context) : context(context)
{
    ...
    configLayout->addWidget(startHttp, 6+offset,1, Qt::AlignLeft);
    offset += 1;
#endif
#ifdef GC_WANT_R
    embedR = new QCheckBox(tr("Enable R"), this);
    embedR->setChecked(appsettings->value(NULL, GC_EMBED_R, true).toBool());
    configLayout->addWidget(embedR, 6+offset,1, Qt::AlignLeft);
    offset += 1;
    ...
    configLayout->addWidget(embedPython, 6+offset,1, Qt::AlignLeft);
    ...
    configLayout->addWidget(opendata, 6+offset,1, Qt::AlignLeft);
    ...
    configLayout->addWidget(athleteLabel, 6 + offset,0, Qt::AlignRight);
    configLayout->addWidget(athleteDirectory, 6 + offset,1);
    configLayout->addWidget(athleteBrowseButton, 6 + offset,2);
    ...
    configLayout->addWidget(workoutLabel, 7 + offset,0, Qt::AlignRight);
    configLayout->addWidget(workoutDirectory, 7 + offset,1);
    configLayout->addWidget(workoutBrowseButton, 7 + offset,2);
    ...
    configLayout->addWidget(rLabel, 7 + offset,0, Qt::AlignRight);
    configLayout->addWidget(rDirectory, 7 + offset,1);
    configLayout->addWidget(rBrowseButton, 7 + offset,2);
    offset++;
    ...
    configLayout->addWidget(pythonLabel, 7 + offset,0, Qt::AlignRight);
    configLayout->addWidget(pythonDirectory, 7 + offset,1);
    configLayout->addWidget(pythonBrowseButton, 7 + offset,2);
    offset++;
    ...
    QWidget(parent), changed(false)
{
    ...
    tabMetricsType = new QTabWidget(this);
    tabMetricsType->addTab(availList, STR_AVAILABLE_METRICS);

    // Create Notio metrics tab list.
    m_notioMetricsList = new QListWidget(this);
    m_notioMetricsList->setSortingEnabled(true);
    m_notioMetricsList->setSelectionMode(QAbstractItemView::SingleSelection);
    tabMetricsType->addTab(m_notioMetricsList, STR_NOTIO_METRICS);

    // Create XData metrics tab list.
    xdataList = new QListWidget(this);
    xdataList->setSortingEnabled(true);
    xdataList->setSelectionMode(QAbstractItemView::SingleSelection);
    tabMetricsType->addTab(xdataList, STR_XDATA_METRICS);

    availLayout->addWidget(tabMetricsType);

    // Create selected metrics list.
    ...
    for (int i = 0; i < factory.metricCount(); ++i) {
        ...
        // Check for the source of the metrics to fill the right list.
        switch (m->sourceType())
        {
        case RideMetric::Notio:
            m_notioMetricsList->addItem(item);
            break;
        case RideMetric::XData:
            xdataList->addItem(item);
            break;
        case RideMetric::Internal:
        default:
    ...
    // Fill the selected metrics list.
    bool wXDataAvailable = false;
    foreach (QString symbol, selectedMetrics) {
        ...
        // We found a XData type metric.
        if (m->sourceType() == RideMetric::XData)
            wXDataAvailable = true;
    }

    // Get the index of the xdataList tab.
    int wIndex = tabMetricsType->indexOf(xdataList);

    // Remove xdataList tab if there are no traces of XData type metrics.
    if ((xdataList->count() == 0) && (wXDataAvailable == false))
        tabMetricsType->removeTab(wIndex);
    ...
    connect(xdataList, SIGNAL(itemSelectionChanged()), this, SLOT(xdataChanged()));
    connect(m_notioMetricsList, SIGNAL(itemSelectionChanged()), this, SLOT(notioMetricsChanged()));
    connect(selectedList, SIGNAL(itemSelectionChanged()),
            this, SLOT(selectedChanged()));

    connect(tabMetricsType, SIGNAL(currentChanged(int)), SLOT(updateCurrentTabSelection(int)));
}
...
IntervalMetricsPage::leftClicked()
{
    ...
    // Get the index of the selected metric by is symbol's name.
    RideMetricFactory &factory = RideMetricFactory::instance();
    int wMetricIndex = factory.allMetrics().indexOf(item->data(Qt::UserRole).toString());

    // Get the available metrics list according to the metric type.
    switch (factory.metricSourceType(wMetricIndex)) {
    case RideMetric::Notio:
        listWidget = m_notioMetricsList;
        break;
    case RideMetric::XData:
        listWidget = xdataList;
        break;
    case RideMetric::Internal:
    default:
        listWidget = availList;
        break;
    }

    // Take selected item and put it back into the available metrics list.
    if (listWidget && selectedList && item)
    {
...
{
    assert(!dynamic_cast<QListWidget*>(tabMetricsType->currentWidget())->selectedItems().isEmpty());
    QListWidgetItem *item = dynamic_cast<QListWidget*>(tabMetricsType->currentWidget())->selectedItems().first();
    dynamic_cast<QListWidget*>(tabMetricsType->currentWidget())->takeItem(dynamic_cast<QListWidget*>(tabMetricsType->currentWidget())->row(item));
    selectedList->addItem(item);
    changed = true;
}
...
{
    qDebug()<<"BestsMetricsPage::availChanged()";
    rightButton->setEnabled(!dynamic_cast<QListWidget*>(tabMetricsType->currentWidget())->selectedItems().isEmpty());
}

void
IntervalMetricsPage::xdataChanged()
{
    qDebug()<<"IntervalMetricsPage::xdataChanged()";
    rightButton->setEnabled(!dynamic_cast<QListWidget*>(tabMetricsType->currentWidget())->selectedItems().isEmpty());
}

void IntervalMetricsPage::notioMetricsChanged() {
    qDebug()<<"IntervalMetricsPage::xdataChanged()";
    rightButton->setEnabled(!dynamic_cast<QListWidget*>(tabMetricsType->currentWidget())->selectedItems().isEmpty());
}

///////////////////////////////////////////////////////////////////////////////
/// \brief IntervalMetricsPage::updateCurrentTabSelection
///        This method refresh the current metrics tab selection open tab
///        changed event.
/// \param[in] iIndex   Index of the metric tab.
///////////////////////////////////////////////////////////////////////////////
void IntervalMetricsPage::updateCurrentTabSelection(int iIndex)
{
    if (iIndex == tabMetricsType->indexOf(availList))
        availChanged();
    else if (iIndex == tabMetricsType->indexOf(xdataList))
        xdataChanged();
    else if (iIndex == tabMetricsType->indexOf(m_notioMetricsList))
        notioMetricsChanged();
}
...
void
BestsMetricsPage::xdataChanged()
{
    qDebug()<<"BestsMetricsPage::xdataChanged()";
    rightButton->setEnabled(!dynamic_cast<QListWidget*>(tabMetricsType->currentWidget())->selectedItems().isEmpty());
}

void BestsMetricsPage::notioMetricsChanged() {
    qDebug()<<"BestsMetricsPage::xdataChanged()";
    rightButton->setEnabled(!dynamic_cast<QListWidget*>(tabMetricsType->currentWidget())->selectedItems().isEmpty());
}

///////////////////////////////////////////////////////////////////////////////
/// \brief BestsMetricsPage::updateCurrentTabSelection
///        This method refresh the current metrics tab selection open tab
///        changed event.
/// \param[in] iIndex   Index of the metric tab.
///////////////////////////////////////////////////////////////////////////////
void BestsMetricsPage::updateCurrentTabSelection(int iIndex)
{
    if (iIndex == tabMetricsType->indexOf(availList))
        availChanged();
    else if (iIndex == tabMetricsType->indexOf(xdataList))
    ...
    QWidget(parent), changed(false)
{
    ...
    // Get selected metric for summary settings.
...
void
SummaryMetricsPage::xdataChanged()
{
    qDebug()<<"SummaryMetricsPage::xdataChanged()";
    rightButton->setEnabled(!dynamic_cast<QListWidget*>(tabMetricsType->currentWidget())->selectedItems().isEmpty());
}

void SummaryMetricsPage::notioMetricsChanged() {
    qDebug()<<"SummaryMetricsPage::xdataChanged()";
    rightButton->setEnabled(!dynamic_cast<QListWidget*>(tabMetricsType->currentWidget())->selectedItems().isEmpty());
}

///////////////////////////////////////////////////////////////////////////////
/// \brief SummaryMetricsPage::updateCurrentTabSelection
///        This method refresh the current metrics tab selection open tab
///        changed event.
/// \param[in] iIndex   Index of the metric tab.
///////////////////////////////////////////////////////////////////////////////
void SummaryMetricsPage::updateCurrentTabSelection(int iIndex)
{
    if (iIndex == tabMetricsType->indexOf(availList))
        availChanged();
    else if (iIndex == tabMetricsType->indexOf(xdataList))
...
ProcessorPage::ProcessorPage(Context *context) : context(context)
{
    ...
    while (i.hasNext()) {
        ...
        // Check for Notio compute data processor.
        bool wNotioProcessor = (i.key() == "_Compute Notio Data");
        ...
        // Notio Data processor cannot be activated during save.
        if (!wNotioProcessor)
        ...
        if (appsettings->value(NULL, GC_QSETTINGS_GLOBAL_GENERAL+configsetting, "Manual").toString() == "Manual")
            ...
            comboButton->setCurrentIndex(wNotioProcessor ? 0 : 2);   // Notio Data processor cannot be activated during save.
        else
...
///////////////////////////////////////////////////////////////////////////////////
/// \brief chartTreeWidget::chartTreeWidget
///        Constructor
///
/// \param[in] iParent  Widget parent.
///////////////////////////////////////////////////////////////////////////////////
ChartTreeWidget::ChartTreeWidget(QWidget *iParent) : QTreeWidget(iParent)
{
    headerItem()->setText(0, tr("Chart Title"));
    setColumnWidth(0, static_cast<int>(200 * dpiXFactor));
    setColumnCount(1);
...
///////////////////////////////////////////////////////////////////////////////////
/// \brief chartTreeWidget::moveUp
///        This method is called when the Up button is clicked.
///////////////////////////////////////////////////////////////////////////////////
void ChartTreeWidget::moveUp()
{
    // Get the selected chart items.
    auto wSelectedCharts = selectedItems();

    if (wSelectedCharts.count())
    {
        // Verify if the first item is already at the top.
        int wIndex = invisibleRootItem()->indexOfChild(wSelectedCharts.first());
        if (wIndex == 0)
            return;

        // Retrieve selected items.
        QList<QTreeWidgetItem *> wItemsToMove;
        for (auto &wCharItr : wSelectedCharts)
        {
            int wIndex = invisibleRootItem()->indexOfChild(wCharItr);
            wItemsToMove.append(invisibleRootItem()->takeChild(wIndex));
        }

        // Insert children one position up.
        invisibleRootItem()->insertChildren(wIndex - 1, wItemsToMove);
        setCurrentItem(wItemsToMove.first());

        // Reselect moved items.
        for (auto &wChartItr : wItemsToMove)
        {
            setItemSelected(wChartItr, true);
        }

        emit treeListChanged();
    }
}

///////////////////////////////////////////////////////////////////////////////////
/// \brief chartTreeWidget::moveDown
///        This method is called when the Down button is clicked.
///////////////////////////////////////////////////////////////////////////////////
void ChartTreeWidget::moveDown()
{
    // Get selected chart items.
    auto wSelectedCharts = selectedItems();

    if (wSelectedCharts.count())
    {
        // Verifty if the last item is already at the bottom.
        int wIndex = invisibleRootItem()->indexOfChild(wSelectedCharts.last());
        if (wIndex == (invisibleRootItem()->childCount()-1))
            return;

        // Get first item index.
        wIndex = invisibleRootItem()->indexOfChild(wSelectedCharts.first());
        ...
        // Insert children one position down.
        invisibleRootItem()->insertChildren(wIndex + 1, wItemsToMove);
        setCurrentItem(wItemsToMove.last());
...
///////////////////////////////////////////////////////////////////////////////////
/// \brief chartTreeWidget::startDrag
///        This method override the startDrag method to force the supported actions
///        of the drag and drop event to only move an item.
///
/// \param[in] iSupportedActions    Supported drop actions.
///////////////////////////////////////////////////////////////////////////////////
void ChartTreeWidget::startDrag(Qt::DropActions iSupportedActions)
{
    // We'll override the actions.
    Q_UNUSED(iSupportedActions)

    // Only the move action is supported.
    Qt::DropActions wMoveOnly = Qt::MoveAction;
    QDrag wDragAction(this);

    // Get selected items.
    m_selectedDragItems = selectedIndexes();
    wDragAction.setMimeData(model()->mimeData(m_selectedDragItems));

    // Move items.
    wDragAction.exec(wMoveOnly);

    emit treeListChanged();
}

///////////////////////////////////////////////////////////////////////////////////
/// \brief chartTreeWidget::dropEvent
///        This method is invoked when a drop event occurs. It reselects the items
///        which were moved and set current item to the first selected one.
///
/// \param[in] iEvent   Drop event that occurred.
///////////////////////////////////////////////////////////////////////////////////
void ChartTreeWidget::dropEvent(QDropEvent *iEvent)
{
    QTreeWidget::dropEvent(iEvent);

    if (m_selectedDragItems.empty())
    {
        // No current item.
    ...
    {
        // Set first selected item as current.
        setCurrentItem(itemFromIndex(m_selectedDragItems.first()));

        // Set selected items; by default, the drop event deselect all.
        for (auto i : m_selectedDragItems)
            setItemSelected(itemFromIndex(i), true);

        m_selectedDragItems.clear();
    }
}

///////////////////////////////////////////////////////////////////////////////////
/// \brief chartTreeWidget::keyPressEvent
///        This method is invoked when the user press a key on the keyboard.
///
/// \param[in] iEvent   Key event that occured.
///////////////////////////////////////////////////////////////////////////////////
void ChartTreeWidget::keyPressEvent(QKeyEvent *iEvent)
{
    qDebug() << Q_FUNC_INFO;

    // Check for arrow up and down key pressed.
    int wKey = iEvent->key();
    switch (wKey)
    {
    // Up key pressed.
    ...
    QTreeWidget::keyPressEvent(iEvent);
}

///////////////////////////////////////////////////////////////////////////////////
/// \brief ChartMngrPage::ChartMngrPage
///        Constructor.
///
/// \param[in] iContext Context pointer.
///////////////////////////////////////////////////////////////////////////////////
ChartMngrPage::ChartMngrPage(Context *iContext) : m_context(iContext)
{
    QVBoxLayout *wMainLayout = new QVBoxLayout(this);
    ...
    m_upButton->setFixedSize(static_cast<int>(20 * dpiXFactor), static_cast<int>(20 * dpiYFactor));
    m_downButton->setFixedSize(static_cast<int>(20 * dpiXFactor), static_cast<int>(20 * dpiYFactor));
#else
...
#endif
    QHBoxLayout *wActionButtons = new QHBoxLayout;
    wActionButtons->setSpacing(static_cast<int>(2 * dpiXFactor));
    wActionButtons->addWidget(m_upButton);
    wActionButtons->addWidget(m_downButton);
    wActionButtons->addStretch();
    wActionButtons->addStretch();

    // Views Tabs widget
    m_viewTabs = new QTabWidget(this);

    // Populates charts list for all views.
    int wViewsCount = m_context->tab->viewsCount();
    for (int index = 0; index < wViewsCount; index++)
    {
        m_tabsOrderChanged[index] = false;

        // Create charts tree widget.
        ChartTreeWidget *wChartsTree = getChartsTree(index);
        QString wViewName = m_context->tab->view(index)->page()->getName();

        // Omit the "Diary" view in case the libical library is not used.
#ifndef GC_HAVE_ICAL
        if (wViewName == tr("Diary"))
        {
            if (wChartsTree)
            {
                wChartsTree->clear();
...
#endif
        // Create view tab charts list.
        m_viewTabs->addTab(wChartsTree, wViewName);
    }

    // Select the current view by default.
    int wTabSelection = tabIndex(m_context->tab->currentView());
    m_viewTabs->setCurrentIndex(wTabSelection);
    m_fields = static_cast<ChartTreeWidget*>(m_viewTabs->currentWidget());

    // Create layout.
    wMainLayout->addWidget(m_viewTabs);
    wMainLayout->addLayout(wActionButtons);

    // Connect Signals to Slots
    connect(m_upButton, SIGNAL(clicked()), m_fields, SLOT(moveUp()));
    connect(m_downButton, SIGNAL(clicked()), m_fields, SLOT(moveDown()));
    connect(m_viewTabs, SIGNAL(currentChanged(int)), this, SLOT(currentTabChanged(int)));
}

///////////////////////////////////////////////////////////////////////////////////
/// \brief ChartMngrPage::currentTabChanged
///        This method is called when the current tab is changed. It updates the
///        charts list with the selected view in the charts manager.
///
/// \param[in] iIndex   Index of the newly selected tab.
///////////////////////////////////////////////////////////////////////////////////
void ChartMngrPage::currentTabChanged(int iIndex)
{
    // Get the view index linked to the charts manager tab index.
    int wViewIndex = viewIndex(iIndex);

    // Check if the view exists.
    if (wViewIndex < m_context->tab->viewsCount())
    {
        // Update the charts list to be displayed.
        m_fields = static_cast<ChartTreeWidget*>(m_viewTabs->currentWidget());
        if (m_fields->invisibleRootItem()->childCount() == 0)
        {
            // Reload from the configuration file if the list is empty.
            m_context->tab->view(wViewIndex)->page()->restoreState();
            m_fields->invisibleRootItem()->addChildren(getChartsTree(wViewIndex)->invisibleRootItem()->takeChildren());
        }
        return;
    }
    qDebug() << "Tab selection out-of-bound.";
}

///////////////////////////////////////////////////////////////////////////////////
/// \brief ChartMngrPage::treeChanged
///        This method is called to indicate the charts list has been modified.
///////////////////////////////////////////////////////////////////////////////////
void ChartMngrPage::treeChanged()
{
    qDebug() << Q_FUNC_INFO;

    // Get the view index linked to the charts manager tab index.
    int wViewIndex = viewIndex(m_viewTabs->currentIndex());

    // Check if the view exists.
    if (wViewIndex < m_context->tab->viewsCount())
    {
        m_tabsOrderChanged[wViewIndex] = true;
        return;
    }
    qDebug() << "Tree changed out-of-bound.";
}

///////////////////////////////////////////////////////////////////////////////////
/// \brief ChartMngrPage::tabIndex
///        This method converts the view index to the charts manager tab index.
///        This is needed because of the libical library which is not necessarily
///        included with the Golden Cheetah build.
///
/// \param[in] iIndex   View index.
/// \return The charts manager tab index.
///////////////////////////////////////////////////////////////////////////////////
int ChartMngrPage::tabIndex(const int iIndex)
{
    // "Diary" view not present
#ifndef GC_HAVE_ICAL
    // If view index is greater or equal to 2 (index of Diary view)
    if (iIndex >= 2)
        return (iIndex - 1);
#endif
    return iIndex;
...
///////////////////////////////////////////////////////////////////////////////////
/// \brief ChartMngrPage::viewIndex
///        This method converts the charts manager tab index to the view index.
///        This is needed because of the libical library which is not necessarily
///        included with the Golden Cheetah build.
///
/// \param[in] iIndex   Charts manager tab index.
/// \return The view index.
///////////////////////////////////////////////////////////////////////////////////
int ChartMngrPage::viewIndex(const int iIndex)
{
    // "Diary" view not present
#ifndef GC_HAVE_ICAL
    // If view index is greater or equal to 2 (index of Diary view)
...
///////////////////////////////////////////////////////////////////////////////////
/// \brief ChartMngrPage::getChartsTree
///        This method builds the charts list to be displayed in the charts
///        manager.
///
/// \param[in] iIndex   View index.
/// \return A QTreeWidget containing the charts list.
///////////////////////////////////////////////////////////////////////////////////
ChartTreeWidget *ChartMngrPage::getChartsTree(const int iIndex)
{
    ChartTreeWidget *wChartsTree = new ChartTreeWidget(this);

    // Get the view charts list.
    const QList<GcChartWindow*> &wViewChartsList = m_context->tab->view(iIndex)->page()->getChartsList();

    if (!wViewChartsList.empty())
    {
        // Build the list.
        for (auto &wViewChartItr : wViewChartsList)
        {
            QTreeWidgetItem *add = new QTreeWidgetItem(wChartsTree);
            add->setFlags((add->flags() & ~Qt::ItemIsDropEnabled) | Qt::ItemNeverHasChildren);
            add->setText(0, wViewChartItr->title());
        }
    }
    connect(wChartsTree, SIGNAL(treeListChanged()), SLOT(treeChanged()));
    return wChartsTree;
}

///////////////////////////////////////////////////////////////////////////////////
/// \brief ChartMngrPage::saveClicked
///        This method saves the modified charts list and update the views.
///
/// \return A state.
///////////////////////////////////////////////////////////////////////////////////
qint32 ChartMngrPage::saveClicked()
{
    // Process all views.
    for (int i = 0; i < m_tabsOrderChanged.size(); i++)
    {
        // Get the charts manager tab.
        int wTabIndex = tabIndex(i);

        // Check if the charts list has been modified.
        if (m_tabsOrderChanged[i])
        {
            const QList<GcChartWindow*> &wViewChartsList = m_context->tab->view(i)->page()->getChartsList();
            ChartTreeWidget *wChartsTree = static_cast<ChartTreeWidget*>(m_viewTabs->widget(wTabIndex));

            // Skip if the modified list doesn't have the same number of items.
            if (wViewChartsList.count() != wChartsTree->topLevelItemCount())
                continue;

            // Process the modified list.
            for (int k = 0; k < wChartsTree->topLevelItemCount(); k++)
            {
                // Find the corresponding chart title in the view.
                for (int j = 0; j < wViewChartsList.count(); j++)
                {
                    if (wChartsTree->topLevelItem(k)->text(0) == wViewChartsList.at(j)->title())
                    {
                        int wToIndex = k;
                        int wFromIndex = j;
                        if (wToIndex != wFromIndex)
                        {
                            // Move the chart in the view if the index is not the same.
                            m_context->tab->view(i)->page()->tabMoved(wFromIndex, wToIndex);
                            break; // Process the next chart in the manager list.
                        }
                    }
                }
            }

            // Save state if the modified list is not the current view.
            if (m_context->tab->currentView() != i)
            {
               m_context->tab->view(i)->page()->saveState();
            }

            // Update the view.
            m_context->tab->view(i)->page()->updateChartsOrderLayout();
        }
    }
    return 0;
}
