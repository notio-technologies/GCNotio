...
#ifdef GC_DEBUG
bool MainWindow::gc_devMode = true;
#else
bool MainWindow::gc_devMode = false;
#endif
...
MainWindow::MainWindow(const QDir &home)
{
...
#ifdef GC_HAVE_NOTIOALGO
    notioToolsMenu = optionsMenu->addMenu("Notio");
    notioToolsMenu->setToolTipsVisible(true);
    connect(optionsMenu, SIGNAL(aboutToShow()), SLOT(setNotioMenuEnabled()));

    // Compute action.
    QAction *wCompute = notioToolsMenu->addAction(tr("Compute"), this, SLOT(NotioMenuCompute()), tr("Ctrl+1"));
    wCompute->setToolTip(tr("Computes ride data. Calculates CdA among others."));

    // Speed correction menu.
    QMenu *speedMenu = notioToolsMenu->addMenu(tr("Speed"));
    speedMenu->setToolTipsVisible(true);

    // Correct wheel size action.
    QAction *wWheelCorrection = speedMenu->addAction(tr("Change Wheel Size"), this, SLOT(NotioMenuSetNewWheelSize()), tr("Ctrl+2"));
    wWheelCorrection->setToolTip(tr("Changes wheel size in the event the device was not configured correctly and computes the ride."));

    // Adjust speed sensor offset action.
    QAction *wSpeedSensorDelay = speedMenu->addAction(tr("Shift Speed Data"), this, SLOT(NotioMenuSpeedSensorDelay()), tr("Ctrl+3"));
    wSpeedSensorDelay->setToolTip(tr("Computes the ride by shifting the speed data to compensate the delay of the sensor."));

    // Altitude compute method menu.
    QMenu *altMenu = notioToolsMenu->addMenu(tr("Altitude"));
    altMenu->setToolTipsVisible(true);

    m_correctedAlt = altMenu->addAction(tr("Corrected"), this, SLOT(NotioMenuSetCorrectAlt()));
    m_correctedAlt->setToolTip(tr("Calculate corrected altitude during data computation."));
    m_correctedAlt->setCheckable(true);
    m_correctedAlt->setChecked(true);

    m_constantAlt = altMenu->addAction(tr("Constant"), this, SLOT(NotioMenuSetZeroAlt()));
    m_constantAlt->setToolTip(tr("Set a constant altitude of %1 meters for the entire ride.").arg(NotioFuncCompute::cAltConstant));
    m_constantAlt->setCheckable(true);
    m_constantAlt->setChecked(false);
    ...
    viewMenu->addSeparator();
#endif
    viewMenu->addAction(tr("Reorder Charts"), this, SLOT(openChartsMngr()));
    viewMenu->addAction(tr("Import Layout..."), this, SLOT(importSavedWindowLayout()));
    viewMenu->addAction(tr("Export Layout..."), this, SLOT(saveWindowLayout()));
    ...
    helpMenu->addSeparator();
    helpMenu->addAction(tr("GC Notio User Guide"), this, SLOT(notioHelpView()));
    helpMenu->addAction(tr("Notio FAQ"), this, SLOT(notioFaq()));
    helpMenu->addAction(tr("Contact Notio Support"), this, SLOT(notioSupport()));
    helpMenu->addSeparator();
    helpMenu->addAction(tr("Check for updates"), this, SLOT(checkUpdates()));
...
///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::setStatusDialogText
///        This method sets the status message dialog text.
///
/// \param[in] iText    Text to display.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::setStatusDialogText(QString iText)
{
    if (m_statusDialog)
    {
        m_statusDialog->setLabelText(iText);
        m_statusDialog->show();
    }
}

///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::setStatusDialogInformativeText
///        This method sets the status message dialog informative text.
///
/// \param[in] iText    Informative text to display.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::setStatusDialogInformativeText(QString iText)
{
    if (m_statusDialog)
    {
        m_statusDialog->setInformativeText(iText);
    }
}

///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::setStatusDialogTitle
///        This methods sets the status message dialog title.
///
/// \param[in] iTitle   Title.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::setStatusDialogTitle(QString iTitle)
{
    if (m_statusDialog)
    {
        m_statusDialog->setWindowTitle(iTitle);
    }
}

///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::setStatusDialogValue
///        This method sets the status message dialog progress bar value.
///
/// \param[in] iValue   Value in percentage.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::setStatusDialogValue(int iValue)
{
    if (m_statusDialog)
    {
        m_statusDialog->setProgressValue(iValue);
    }
}

///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::setStatusDialog
///        This method creates the status message dialog.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::setStatusDialog()
{
    // New frameless widget that will show messages to the user.
    if (m_statusDialog == nullptr)
    {
        m_statusDialog = new ProgressMessageDialog("", "", tr("This could take a few minutes."), this);

        connect(currentTab->context, SIGNAL(configChanged(qint32)), m_statusDialog, SLOT(setWindowDisplayType()));
        connect(this, SIGNAL(closingWindow()), SLOT(clearStatusDialog()));
...
///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::clearStatusDialog
///        This method closes the status message dialog and delete it.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::clearStatusDialog()
{
    if (m_statusDialog)
    {
...
#if defined(_MSC_VER) && defined(_WIN64)
///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::setTaskBarProgressValue
///        This method sets the Windows task bar button progress value.
///
/// \param[in] iValue   Value in percentage.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::setTaskBarProgressValue(int iValue)
{
    if (m_winTaskBar)
    {
        // Get the progres object and display the progress.
        QWinTaskbarProgress *wProgress = m_winTaskBar->progress();
        wProgress->setVisible(true);
        wProgress->setValue(iValue);
    }
}

///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::setTaskBarProgressShow
///        This method shows the Windows task bar progress.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::setTaskBarProgressShow()
{
    if (m_winTaskBar)
    {
        QWinTaskbarProgress *wProgress = m_winTaskBar->progress();
...
///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::setTaskBarProgressHide
///        This method hides the Windows task bar progress.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::setTaskBarProgressHide()
{
    if (m_winTaskBar)
    {
        QWinTaskbarProgress *wProgress = m_winTaskBar->progress();
        wProgress->setVisible(false);
    }
}
#endif
...
MainWindow::setChartMenu()
{
    ...
    for(int i=0; GcWindows[i].relevance; i++) {
        // Hide development charts from chart menu.
        if ((GcWindows[i].relevance & VIEW_DEV) && !gc_devMode)
            continue;
...
MainWindow::eventFilter(QObject *o, QEvent *e)
{
    if (o == this) {
        ...
        {
#if defined(_MSC_VER) && defined(_WIN64)
            // Create Windows task bar button.
            if (m_winTaskBar == nullptr)
            {
                m_winTaskBar = new QWinTaskbarButton(this);
                m_winTaskBar->setWindow(windowHandle());
            }
#endif
        }
    }
...
///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::checkUpdates
///        This method is called by the user to force a verification of new
///        versions available. On automatic checks, the user can choose to
///        inhibit to automatic process. In that case, there are no way for him
///        to know if a new version is available besides checking GoldenCheetah
///        website.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::checkUpdates()
{
    // Reset last check date and set checked version to the current build.
    appsettings->setValue(GC_LAST_NK_VERSION_CHECK_DATE, QDate(1990, 1, 1));
    appsettings->setValue(GC_LAST_NK_VERSION_CHECKED, NK_VERSION_LATEST);

    // Set flag for manual check.
    versionClient->manualCheckUpdates();

    // Verify for new available versions.
    versionClient->informUserAboutLatestVersions();
}
...
#ifdef GC_HAVE_NOTIOALGO
void MainWindow::NotioMenuCompute()
{
    qDebug() << Q_FUNC_INFO;
    ...
    if (currentTab)
    {
        ...
        {
            RideItem *wRide = currentTab->context->ride;
            NotioFuncCompute compute(wRide);
            compute();
        }
    }
}
...
///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::NotioMenuSetNewWheelSize
///        This method displays a dialog window to change the wheel size for a
///        Notio ride.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::NotioMenuSetNewWheelSize()
{
    RideItem *wThisRide = this->currentTab->context->ride;
    if (wThisRide)
    {
        RideFile *wRide = wThisRide->ride();
        if (wRide)
        {
            QFontMetrics metrics(QApplication::font());

            // Create dialog window.
            QDialog *wCorrectWheelSizeDialog = new QDialog(this, Qt::Dialog);
            QString wDialogTitle = tr("Corrects wheel size");
            int wTitleWidth = metrics.width(wDialogTitle) + 10;
            wCorrectWheelSizeDialog->setWindowTitle(wDialogTitle);

            wCorrectWheelSizeDialog->setModal(true);

            // Layout for the original wheel size data.
            QHBoxLayout *wOriginalLayout = new QHBoxLayout;

            // Text label.
            QString wOriginalText = tr("Original Value:");

            int wLabelWidth = metrics.width(wOriginalText) + 10;
            QLabel *wOriginalLabel = new QLabel(wOriginalText);
            wOriginalLabel->setWhatsThis(tr("Wheel size configuration during ride."));
            wOriginalLayout->addWidget(wOriginalLabel);

            // Value label.
            QString wOriginalValue = wRide->getTag("wheelCirc", "");
            int wValueWidth = metrics.width(wOriginalValue) + 10;
            QLabel *wOriginalValueLabel = new QLabel(QString(tr("%1 mm")).arg(wOriginalValue));
            wOriginalLayout->addWidget(wOriginalValueLabel, 0, Qt::AlignRight);

            // Layout for the new wheel size data.
            QHBoxLayout *wNewLayout = new QHBoxLayout;
            QString wNewSavedValue = wRide->getTag("customTireSize", "");

            // Text label.
            QLabel *wNewValueLabel = new QLabel(tr("New Value:"));
            wNewValueLabel->setFixedWidth(wLabelWidth);
            wNewValueLabel->setWhatsThis(tr("Corrected wheel size value."));
            wNewLayout->addWidget(wNewValueLabel);

            // Value edit field.
            QLineEdit *wNewWheelSize = new QLineEdit(wNewSavedValue);
            wNewWheelSize->setValidator(new QIntValidator(100, 5000));
            wNewWheelSize->setFixedWidth(wValueWidth);
            wNewWheelSize->setAlignment(Qt::AlignRight);
            wNewWheelSize->setWhatsThis(tr("Enter a new wheel circumference in millimeters to reflect your actual wheel used during ride."));
            wNewLayout->addSpacing(static_cast<int>((wTitleWidth - wLabelWidth) * dpiXFactor));
            wNewLayout->addWidget(wNewWheelSize, 0, Qt::AlignRight);

            // Units text label.
            QLabel *wUnitsLabel = new QLabel(tr("mm"));
            wNewLayout->addWidget(wUnitsLabel, 0, Qt::AlignRight);

            // Layout for the buttons.
            QHBoxLayout *wButtonLayout = new QHBoxLayout;

            // Cancel button.
            QPushButton *wCancelButton = new QPushButton(tr("Cancel"));
            connect(wCancelButton, SIGNAL(clicked(bool)), wCorrectWheelSizeDialog, SLOT(reject()));
            wButtonLayout->addStretch(1);
            wButtonLayout->addWidget(wCancelButton, 0, Qt::AlignRight);

            // Compute button.
            QPushButton *wComputeButton = new QPushButton(tr("Compute"));
            wComputeButton->setWhatsThis(tr("Save new wheel size value and compute data."));
            connect(wComputeButton, SIGNAL(clicked(bool)), wCorrectWheelSizeDialog, SLOT(accept()));
            wButtonLayout->addWidget(wComputeButton, 0, Qt::AlignRight);

            // Create main layout.
            QVBoxLayout *wMainLayout = new QVBoxLayout;

            wMainLayout->addLayout(wOriginalLayout);
            wMainLayout->addLayout(wNewLayout);
            wMainLayout->addLayout(wButtonLayout);
            wCorrectWheelSizeDialog->setLayout(wMainLayout);

            // Execute dialog window.
            int wResult = wCorrectWheelSizeDialog->exec();

            // Set new wheel circumference and compute the ride's data.
            if (wResult == QDialog::Accepted)
            {
                wRide->command->startLUW(QString(tr("Correct wheel size to %1 mm")).arg(wNewWheelSize->text()));

                // Save new value.
                wRide->setTag("customTireSize", wNewWheelSize->text());

                // For compatibility issues, rides computed with this version will be compatible with previous GC version.
                // To be removed in future release.
                wRide->setTag("notio.newWheelCirc", wNewWheelSize->text());

                // Recompute speed and ride data.
                NotioFuncCompute compute(wThisRide);
                compute("SPEED");

                wRide->command->endLUW();
            }
            return;
        }
    }
    QMessageBox::critical(this, tr("Correct wheel size for the ride"), tr("No ride is selected. Please select one before trying to change wheel size."));
}

///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::NotioMenuSpeedSensorDelay
///        This method displays a dialog window to the user to enter an offset
///        in seconds to compensate the delay of the speed sensor.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::NotioMenuSpeedSensorDelay()
{
    RideItem *wThisRide = this->currentTab->context->ride;
    if (wThisRide)
    {
        ...
        if (wRide)
        {
            ...
            // Create dialog window.
            QDialog *wAdjustSpeedSensorDialog = new QDialog(this, Qt::Dialog);
            QString wDialogTitle = tr("Shift Speed Data");
            int wTitleWidth = metrics.width(wDialogTitle) + 10;
            wAdjustSpeedSensorDialog->setWindowTitle(wDialogTitle);

            wAdjustSpeedSensorDialog->setModal(true);

            // Layout for the speed data delay.
            QHBoxLayout *wValueEntryLayout = new QHBoxLayout;
            double wDelayValue = wRide->getTag("notio.speedSensorDelay", "0").toDouble();

            // Text label.
            QLabel *wDelayValueLabel = new QLabel(tr("Delay Value:"));
            int wLabelWidth = metrics.width(wDelayValueLabel->text()) + 10;
            wDelayValueLabel->setWhatsThis(tr("Delay value of the speed sensor."));
            wValueEntryLayout->addWidget(wDelayValueLabel);

            // Value spin box field.
            QDoubleSpinBox *wSpeedDelay = new QDoubleSpinBox;
            wSpeedDelay->setValue(wDelayValue);
            wSpeedDelay->setRange(0, 5);
            wSpeedDelay->setSingleStep(0.25);
            wSpeedDelay->setSuffix(tr(" seconds"));
            wSpeedDelay->setDecimals(2);
            wSpeedDelay->setAlignment(Qt::AlignRight);
            wSpeedDelay->setWhatsThis(tr("Enter the speed delay observed to align the data with Notio data."));
            wValueEntryLayout->addSpacing(static_cast<int>((wTitleWidth - wLabelWidth) * dpiXFactor));
            wValueEntryLayout->addWidget(wSpeedDelay, 0, Qt::AlignRight);
            ...
            // Cancel button.
            QPushButton *wCancelButton = new QPushButton(tr("Cancel"));
            connect(wCancelButton, SIGNAL(clicked(bool)), wAdjustSpeedSensorDialog, SLOT(reject()));
            wButtonLayout->addStretch(1);
            wButtonLayout->addWidget(wCancelButton, 0, Qt::AlignRight);

            // Compute button.
            QPushButton *wComputeButton = new QPushButton(tr("Compute"));
            wComputeButton->setWhatsThis(tr("Save delay value and compute data."));
            connect(wComputeButton, SIGNAL(clicked(bool)), wAdjustSpeedSensorDialog, SLOT(accept()));
            wButtonLayout->addWidget(wComputeButton, 0, Qt::AlignRight);

            // Create main layout.
            QVBoxLayout *wMainLayout = new QVBoxLayout;

            wMainLayout->addLayout(wValueEntryLayout);
            wMainLayout->addLayout(wButtonLayout);
            wAdjustSpeedSensorDialog->setLayout(wMainLayout);

            // Execute dialog window.
            int wResult = wAdjustSpeedSensorDialog->exec();

            // Set the delay value and compute the ride's data.
            if (wResult == QDialog::Accepted)
            {
                wRide->command->startLUW(QString(tr("Shift speed data according to a delay of %1")).arg(wSpeedDelay->value()));

                // Save value.
                wRide->setTag("notio.speedSensorDelay", QString::number(wSpeedDelay->value()));
    ...
    QMessageBox::critical(this, tr("Shift speed sensor data for the ride"), tr("No ride is selected. Please select one before trying to align speed sensor data."));
}

///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::NotioMenuSetCorrectAlt
///        This method calls the process to compute the current ride with the
///        corrected altitude.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::NotioMenuSetCorrectAlt()
{
    Context *wContext = this->currentTab->context;
    if (wContext)
    {
        RideItem *wThisRide = this->currentTab->context->ride;
        RideFile *wRide = wThisRide->ride();
        ...
        GcChartWindow *wCurrentChart = currentTab->view(wContext->viewIndex)->page()->currentChart();

        // Check if the current chart type is Notio CdA Analysis window.
        if (wCurrentChart->type() == GcWinID::NotioCDAWindow)
        {
            // Display a warning message if there are unsaved parameters.
            if (static_cast<NotioCDAWindow *>(wCurrentChart)->hasParamToSave())
            {
                QString wDialogText = tr("Your are about to compute the current ride with the corrected altitude. Unsaved "
                                         "CdA Analysis parameter changes will be lost.\n\nDo you want to continue?");

                QMessageBox *wAltDialog = new QMessageBox(QMessageBox::Warning, tr("Set Corrected Altitude"), wDialogText, QMessageBox::Yes | QMessageBox::No);
                wAltDialog->setModal(true);

                // The user aborted the operation.
                if (wAltDialog->exec() & QMessageBox::No)
                    return;
            }
        }

        wRide->setTag("notio.altSource","altCompute");
        wRide->setTag("notio.altMethod","altEKF");
...
///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::NotioMenuSetZeroAlt
///        This method calls the process to compute the current ride with a
///        constant altitude.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::NotioMenuSetZeroAlt()
{
    Context *wContext = currentTab->context;
    if (wContext)
    {
        RideItem *wThisRide = this->currentTab->context->ride;
        RideFile *wRide = wThisRide->ride();
        ...
        if (wCurrentChart->type() == GcWinID::NotioCDAWindow)
        {
            ...
            if (static_cast<NotioCDAWindow *>(wCurrentChart)->hasParamToSave())
            {
                QString wDialogText = tr("Your are about to compute the current ride with a constant altitude. Unsaved "
                                         "CdA Analysis parameter changes will be lost.\n\nDo you want to continue?");

                QMessageBox *wAltDialog = new QMessageBox(QMessageBox::Warning, tr("Set Constant Altitude"), wDialogText, QMessageBox::Yes | QMessageBox::No);
                wAltDialog->setModal(true);
        ...
        wRide->setTag("notio.altSource","altCompute");
        wRide->setTag("notio.altMethod","altZero");
...
///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::setNotioMenuEnabled
///        This method is called when the Notio menu is about to be displayed.
///        It enable the menu in the case the activity contains Notio data
///        (BCVX or ROW). The menu won't even show if the software is built
///        without the Notio Algorithms library.
///
///        It is also used to update the altitude compute mode menu items.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::setNotioMenuEnabled()
{
    bool wEnableMenu = false;
    RideItem *wRideItem = currentTab->context->rideItem();
    if (wRideItem && wRideItem->ride())
    {
        // Get altitude mode.
        bool wEkfMode = (wRideItem->ride()->getTag("notio.altMethod", "altEKF") == "altEKF");

        // Update constant altitude action menu.
        QString wConstantStr = currentTab->context->athlete->useMetricUnits ? tr("meters") : tr("feet");
        double wConstantValue = NotioFuncCompute::cAltConstant * (currentTab->context->athlete->useMetricUnits ? 1 : static_cast<double>(FEET_PER_METER));

        m_constantAlt->setToolTip(tr("Set a constant altitude of %1 %2 for the entire ride.").arg(wConstantValue).arg(wConstantStr));
        m_constantAlt->setChecked(!wEkfMode);

        // Update corrected altitude action menu.
        m_correctedAlt->setChecked(wEkfMode);

        // Enable Notio menu.
        wEnableMenu = ((wRideItem->ride()->xdata("BCVX") != nullptr) || (wRideItem->ride()->xdata("ROW") != nullptr));
    }
    notioToolsMenu->setEnabled(wEnableMenu);
}
#endif
...
///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::notioHelpView
///        This method launches the GC Notio help webpage.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::notioHelpView()
{
    QDesktopServices::openUrl(QUrl("https://notio.ai/gcnotio_userguide"));
}

///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::notioFaq
///        This method launches the GC Notio FAQ webpage.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::notioFaq()
{
    QDesktopServices::openUrl(QUrl("https://notio.ai/apps/help-center"));
}

///////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::notioSupport
///        This method launches the Notio Contact Us webpage.
///////////////////////////////////////////////////////////////////////////////
void MainWindow::notioSupport()
{
    QDesktopServices::openUrl(QUrl("https://notio.ai/pages/contact-us-desk"));
}
...
///////////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::openChartsMngr
///        This method creates and opens the charts manager window.
///////////////////////////////////////////////////////////////////////////////////
void MainWindow::openChartsMngr()
{
    ChartsMngrDialog *chartsMngrWindow = new ChartsMngrDialog(currentTab->context);
    chartsMngrWindow->show();
}

///////////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::saveWindowLayout
///        This method saves the current view layout.
///////////////////////////////////////////////////////////////////////////////////
void MainWindow::saveWindowLayout()
{
    // Get the view index
    int wViewIndex = currentTab->currentView();

    // Call the method to save the layout as a file.
    currentTab->view(wViewIndex)->saveState(true);
}

///////////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::importSavedWindowLayout
///        This method imports a previously saved view layout.
///////////////////////////////////////////////////////////////////////////////////
void MainWindow::importSavedWindowLayout()
{
    // Get the view index.
    int wViewIndex = currentTab->currentView();

    // Call the method to save the layout as a file.
    currentTab->view(wViewIndex)->importLayout();
}
...
MainWindow::importFile()
{
    ...
    // Give a little time for the dialog to close before files importation.
    QEventLoop wLoop;
    QTimer::singleShot(100, &wLoop, SLOT(quit()));
    wLoop.exec();
...
void
MainWindow::saveRideEx(RideItem *ride, Context *context)
{
    // no ride
    if (ride == NULL) {
        QMessageBox oops(QMessageBox::Critical, tr("No Activity To Save"),
...
{
    if(action->data().toString().contains("NotioUploadFileAction", Qt::CaseSensitive)) {
        QString wServiceId = action->data().toString().split(',').at(0);

        CloudService *db = CloudServiceFactory::instance().newService(wServiceId, currentTab->context);
        db->uploadFileDialog();
        delete db;
    }

    // upload current ride, if we have one
    else if (currentTab->context->ride) {
        ...
        if (action->text() == "University of Kent") {
            ...
            if (action->data().toString().contains("Notio", Qt::CaseSensitive))
            {
                QString wKey = (action->data().toString() == "NotioKonect") ? GC_NOTIOKONECT_ATHLETE_NAME : "";
                QString wAthleteName = appsettings->cvalue(currentTab->context->athlete->cyclist, wKey, "").toString();

                // Warning to a coach that is about to upload the current ride to a
                // specific athlete's cloud account.
                if (!wAthleteName.isEmpty() && !wAthleteName.contains("Coach"))
                {
                    QString wInfoMsg = QString(tr("You are about to upload the current ride to the athlete <b>%1</b>.<br><br>Do you want to continue?")).arg(wAthleteName);
                    if (QMessageBox::Abort == QMessageBox::information(this, tr("Upload Current Ride"), wInfoMsg, QMessageBox::Ok, QMessageBox::Abort))
                        return;
                }
            }
            CloudService *db = CloudServiceFactory::instance().newService(action->data().toString(), currentTab->context);
...
MainWindow::syncCloud(QAction *action)
{
    ...
    connect(this, SIGNAL(closingWindow()), &sync, SLOT(close()));
    sync.exec();
    delete db;
}
...
///////////////////////////////////////////////////////////////////////////////////
/// \brief MainWindow::messageInfoBox
///        This method display a message box to display information.
///
/// \param[in] iMessage Message to display.
/// \param[in] iTitle   Message box title.
///////////////////////////////////////////////////////////////////////////////////
void MainWindow::messageInfoBox(QString iMessage, QString iTitle)
{
    QMessageBox Msgbox(QMessageBox::Information, iTitle, iMessage);
...
MainWindow::setUploadMenu()
{
    ...
    foreach(QString name, CloudServiceFactory::instance().serviceNames()) {
        ...
        if (s->capabilities() & CloudService::Upload) {
            ...
            // Logic to be reviewed when the option to upload a specific file will be activated.
            if (s->id().contains("Notio", Qt::CaseSensitive)) {
                m_notioUploadMenu = uploadMenu->addMenu(s->uiName() + tr("..."));

                //TODO test active ride here and add menu eventually or gray it out

                // we need the technical name to identify the service to be called
                QAction *uploadCurrentRideAction = new QAction(nullptr);
                uploadCurrentRideAction->setText(tr("Upload Current Ride"));
                uploadCurrentRideAction->setData(s->id());
                m_notioUploadMenu->addAction(uploadCurrentRideAction);

                // we need the technical name to identify the service to be called
                QAction *uploadFileAction = new QAction(nullptr);
                uploadFileAction->setText(tr("Upload file..."));
                uploadFileAction->setData( s->id() + "," + "NotioUploadFileAction");
                m_notioUploadMenu->addAction(uploadFileAction);
            }
            else {
                // we need the technical name to identify the service to be called
